	...
	do
	{
		...
		if ( !szSaveToFile )
		{
			...
			if ( dwArraySize < dwNewDataSize )
			{
				...
				LPSTR pNewData = new char [ dwNewDataSize + 1000001 ];
...
bool HTTPComm::HandleReply3( const char *szReply )
{
	char str [ 256 ];

	if ( bCancelTransfer ) return false;

	if ( !szReply || *szReply == '\0' ) 
	{
		strcpy_s( str, 256, szLastError );
		strcat_s( str, 256, "\r\n\r\n" );
		strcat_s( str, 256, "The server did not reply. Please try again." );
		MessageBox( NULL, str, theApp.m_szErrorString, 0 );
		strcpy_s( szLastError, 256, str );
		if ( szReply ) delete [] szReply;
		return false;
	}
...
void HTTPComm::RefreshSession( )
{
	if ( !Connect( "www.thegamecreators.com", false ) ) return;

	DWORD dwWritten = 0;
	const char* reply = RequestData( "xt/xt_refresh_session.php", NULL, 0, &dwWritten );
...
bool HTTPComm::Login2( LPCSTR szUsername, LPCSTR szPassword )
{
	if ( !szUsername ) return false;
	if ( !szPassword ) return false;

	if ( !Connect( "www.thegamecreators.com", true ) )
	{
		MessageBox( NULL, szLastError, theApp.m_szErrorString, 0 );
		return false;
	}
	...
	DWORD dwWritten = 0;
	const char* reply = RequestData( "xt/xt_login_ex.php", request, (DWORD) strlen(request), &dwWritten );

	Disconnect( );

	if ( !HandleReply3( reply ) ) return false;

	//MessageBox( NULL, reply, "Response", 0 );

	if ( strncmp( reply, "<success>", strlen("<success>") ) != 0 )
	{
		char* szErrorMessage = new char [ dwWritten + 1 ];
		strncpy_s( szErrorMessage, dwWritten + 1, reply, dwWritten );
		szErrorMessage [ dwWritten ] = '\0';

		MessageBox( NULL, szErrorMessage, "Login Error", 0 );

		if ( theApp.m_szUploadKey ) delete [] theApp.m_szUploadKey;
		theApp.m_szUploadKey = NULL;
	...
	{
		const char* szUploadKey = reply + strlen("<success>");
		DWORD dwLength = dwWritten - strlen("<success>");

		if ( theApp.m_szUploadKey ) delete [] theApp.m_szUploadKey;

		theApp.m_szUploadKey = new char [ dwLength + 1 ];
		strncpy_s( theApp.m_szUploadKey, dwLength + 1, szUploadKey, dwLength );
		theApp.m_szUploadKey [ dwLength ] = '\0';
...
bool HTTPComm::UploadPreviewImage( DWORD dwObjID, const char *szFilename, bool bEditing )
{
	...
	if ( error )
	{
		...
		sprintf_s( str, MAX_PATH + 32, "%s: %s", GetLanguageData( _T("TGC Store"), _T("PreviewImageFailed") ).GetString(), szFilename );
...
LPCSTR HTTPComm::GetClubLimits ( LPCSTR szKey, int iClubID )
{
	bCancelTransfer = false; 

	//open connection
	if ( !Connect( "www.thegamecreators.com", false ) ) return NULL;

	char szClubID[11];
	sprintf_s( szClubID, 11, "%d", iClubID );

	char request [ 1500 ];
	strcpy_s( request, 1500, "key=" );
	strcat_s( request, 1500, szKey );
	strcat_s( request, 1500, "&club=" );
	strcat_s( request, 1500, szClubID );

	DWORD dwWritten = 0;
	const char* reply = RequestData( "xt/xt_club_get_limits.php", request, (DWORD) strlen( request ), &dwWritten );

	// return data in ptr
	return reply;
}

LPCSTR HTTPComm::GetClubList ( LPCSTR szKey )
{
	bCancelTransfer = false; 
	...
	DWORD dwWritten = 0;
	const char* reply = RequestData( "xt/xt_club_get_clubs.php", request, (DWORD) strlen( request ), &dwWritten );

	// return data in ptr
	return reply;
}

LPCSTR HTTPComm::UploadToClubCheck ( LPCSTR szKey, LPCSTR szDescription, LPCSTR szFilename, int iClubID )
{
	bCancelTransfer = false; 

	//open main file upload
	FILE *pFile = NULL;
	int error = fopen_s( &pFile, szFilename, "rb" );
	if ( error )
	{
		sprintf_s( szLastError, 1024, "Error opening main file." );
		return NULL;
	}
	...
	DWORD dwHeaderSize = 0;
	char pHeaderString [ 256 ]; //request header
	DWORD dwDataReturned = 0;
	LPSTR pDataReturned = NULL; //server reply
	...
	//how long is the post data in total
	DWORD dwLength = sData.GetLength() + sEnd.GetLength();
	...
	// prepare request
	HINTERNET hHttpRequest = HttpOpenRequest( hInetConnect, szType, "/xt/xt_club_upload_check.php", "HTTP/1.1", "The Agent", NULL, dwFlag, 0 );
	if ( !hHttpRequest )
	{
		strcpy_s( szLastError, 1024, "Failed to open request" );
		delete [] pData;
		return NULL;
	}
	...
	if ( !bSendResult )
	{
		sprintf_s( szLastError, 1024, "Failed to send request: %d", ::GetLastError( ) );
		delete [] pData;
		return NULL;
	}
	...
	//send request data a chunk at a time, allows progress bar to update
	while ( dwRemaining > 50000 && !bCancelTransfer )
	{
		bSendResult = InternetWriteFile( hHttpRequest, pPtr, 50000, &dwWritten );
		if ( !bSendResult )
		{
			sprintf_s( szLastError, 1024, "Failed to send data: %d", ::GetLastError( ) );
			delete [] pData;
			return NULL;
		}
		...
		dwWritten = dwLength - dwRemaining;
		if ( m_pProgressBar && !bCancelTransfer ) m_pProgressBar->SetPos( (int) (dwWritten*100.0f / dwLength) ); //update progress bar
	}

	if ( bCancelTransfer ) 
	{
		strcpy_s( szLastError, 1024, "Upload Cancelled." );
		delete [] pData;
		return NULL;
	}
	...
	if ( !bSendResult )
	{
		sprintf_s( szLastError, 1024, "Failed to end data: %d", ::GetLastError( ) );
		return NULL;
	}
	...
	do
	{
		...
		{
			// error getting chunk
			strcpy_s( szLastError, 1024, "Failed to get chunk" );
			return NULL;
		}
		...
		{
			// recreate a bigger array and add data to it
			DWORD dwNewDataSize = dwDataReturned + dwDownloaded;
...
LPCSTR HTTPComm::UploadToClub ( LPCSTR szKey, LPCSTR szDescription, LPCSTR szFilename, LPCSTR szThumbnail, int iClubID )
{
	bCancelTransfer = false; 
	...
	//open thumbnail file
	error = fopen_s( &pFile, szThumbnail, "rb" );
	if ( error )
	{
		sprintf_s( szLastError, 1024, "Error opening thumbnail file." );
		return NULL;
	}

	//get file size
	fseek( pFile, 0, SEEK_END );
	long lSize2 = ftell( pFile );
	rewind( pFile );

	//get file data
	char *pThumbnailData = new char [ lSize2 ];
	fread( pThumbnailData, 1, lSize2, pFile );
	fclose( pFile );
	pFile = NULL;
	...
	//file data goes here, fill it in later
	...
	//thumbnail data goes here, fill it in later

	//header end
	CString sEnd = "\r\n--------------------AaB03x--\r\n";

	//how long is the post data in total
	DWORD dwLength = sData.GetLength() + lSize + sMiddle.GetLength() + lSize2 + sEnd.GetLength();
	
	//construct post data with sData + file + sMiddle + thumbnail + sEnd
	char *pData = new char [ dwLength ];
	memcpy( pData, sData.GetBuffer(), sData.GetLength() );

	char *pOffset = pData + sData.GetLength();
	memcpy( pOffset, pFileData, lSize );

	pOffset += lSize;
	memcpy( pOffset, sMiddle.GetBuffer(), sMiddle.GetLength() );

	pOffset += sMiddle.GetLength();
	memcpy( pOffset, pThumbnailData, lSize2 );
	...
	// prepare request
	HINTERNET hHttpRequest = HttpOpenRequest( hInetConnect, szType, "/xt_up/xt_club_upload_ex.php", "HTTP/1.1", "The Agent", NULL, dwFlag, 0 );
	if ( !hHttpRequest )
	{
		strcpy_s( szLastError, 1024, "Failed to open request" );
		return NULL;
	...
	while ( dwRemaining > 50000 && !bCancelTransfer )
	{
		...
		dwWritten = dwLength - dwRemaining;
		if ( m_pProgressBar ) m_pProgressBar->SetPos( (int) (dwWritten*100.0f / dwLength) ); //update progress bar
	}

	if ( bCancelTransfer ) 
	{
		InternetCloseHandle( hHttpRequest );
		strcpy_s( szLastError, 1024, GetLanguageData( _T("TGC Store"), _T("UploadCancelled") ) );
		return NULL;
	}
