...
cProperty::cProperty(CObjectProperty* pProperty)
{
	...
	m_hEntityDataFile = OpenFileMapping ( FILE_MAP_ALL_ACCESS, FALSE, "ENTITYPROPERTIES" );
	if ( !m_hEntityDataFile )
	{		
		m_hEntityDataFile = CreateFileMapping( INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 20480, "ENTITYPROPERTIES" );
	}
    
    if ( !m_hEntityDataFile )
    {
        AfxMessageBox( _T("Error Creating Entity Properties File") );
        exit(1);
    }
    
    m_pRawDataPtr = (DWORD*) MapViewOfFile( m_hEntityDataFile, FILE_MAP_WRITE|FILE_MAP_READ, 0, 0, 0);
        
    if ( !m_pRawDataPtr ) 
    {
        AfxMessageBox( _T("Error Getting Entity Properties File") );
        exit(1);
    }

	m_pEntityDataPtr = m_pRawDataPtr+2;

	memset( m_pEntityDataPtr, 0, 20000 );
}

cProperty::~cProperty()
{
	if ( m_pRawDataPtr )  UnmapViewOfFile ( m_pRawDataPtr );
	if ( m_hEntityDataFile ) CloseHandle     ( m_hEntityDataFile );

	m_pRawDataPtr = NULL;
	m_pEntityDataPtr = NULL;
	m_hEntityDataFile = NULL;
}
...
void cProperty::HandleMessages ( cIPC* pIPC, cMessages* pMessage )
{
	...
	if ( !pIPC->SkipIfEventNotWaiting () && *(m_pEntityDataPtr) != 1 )
		return;
...
char* cProperty::GetFileMapString( int* pPos )
{
	int pos = *pPos;
	pos *= 4;

	DWORD* file = m_pEntityDataPtr;
	char* cfile = (char*)file;
    
	int len = *((int*)(cfile+pos));
	pos += 4;
...
int cProperty::NewGroup( int iFilePos )
{
	//char str [ 256 ];
	//sprintf_s( str, "New Group: %s", GetFileMapString( &pos ) );
	//MessageBox( str );

	CString sGroup = GetFileMapString( &iFilePos );
	m_pPropertyWindow->AddGroup ( sGroup );
	...
	while ( *(m_pEntityDataPtr + iFilePos) )
	{
		sField = "";
		sData = "";
		sDesc = "";
		...
		switch( *(m_pEntityDataPtr + iFilePos) )
		{
			case 2: //add group
			{
				iFilePos++;
				iFilePos = NewGroup( iFilePos );
			} break;
			case 3:	//add edit box
			{
				iFilePos++;
				int group = *(m_pEntityDataPtr + iFilePos); iFilePos++;
				sField = GetFileMapString( &iFilePos ); 
				sData = GetFileMapString( &iFilePos );
				sDesc = GetFileMapString( &iFilePos );
				m_pPropertyWindow->AddControl ( 0, group, 0, sField, sData, sDesc, sExtra, sDir, sArr1, sArr2 );
			} break;
			case 4: //add color picker
			{
				iFilePos++;
				int group = *(m_pEntityDataPtr + iFilePos); iFilePos++;
				sField = GetFileMapString( &iFilePos ); 
				sData = GetFileMapString( &iFilePos );
				sDesc = GetFileMapString( &iFilePos );
				m_pPropertyWindow->AddControl ( 2, group, 0, sField, sData, sDesc, sExtra, sDir, sArr1, sArr2 );
			} break;
			case 5: //add file picker
			{
				iFilePos++;
				int group = *(m_pEntityDataPtr + iFilePos); iFilePos++;
				sField = GetFileMapString( &iFilePos ); 
				sData = GetFileMapString( &iFilePos );
				sDesc = GetFileMapString( &iFilePos );
				sDir = GetFileMapString( &iFilePos ); 
				sArr1 = GetFileMapString( &iFilePos );
				sArr2 = GetFileMapString( &iFilePos );
				m_pPropertyWindow->AddControl ( 3, group, 0, sField, sData, sDesc, sExtra, sDir, sArr1, sArr2 );
			} break;
			case 6:	//add list box
			{
				iFilePos++;
				int group = *(m_pEntityDataPtr + iFilePos); iFilePos++;
				int control = *(m_pEntityDataPtr + iFilePos); iFilePos++;
				sField = GetFileMapString( &iFilePos ); 
				sData = GetFileMapString( &iFilePos );
				sDesc = GetFileMapString( &iFilePos );
				m_pPropertyWindow->AddControl ( 1, group, control, sField, sData, sDesc, sExtra, sDir, sArr1, sArr2 );
				int listmax = *(m_pEntityDataPtr + iFilePos); iFilePos++;
				for (int i = 0; i <= listmax; i++ )
				{
					sListItem = GetFileMapString( &iFilePos );
					m_pPropertyWindow->AddItemToListBox ( group, control, sListItem );
				}
			} break;
		}
	}
...
void cProperty::CheckForNewControls2 ( void )
{
	if ( *(m_pEntityDataPtr) != 1 ) return;
