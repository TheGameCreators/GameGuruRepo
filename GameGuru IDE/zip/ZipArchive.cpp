                             ...
                             unsigned long nBufSize)
{
	...
	// FIX FROM PAUL - 010610
	//don't use full path when opening file, we are already in the current directory
	LPCSTR szFilename = lpszFilePath;
	if ( 0 )
	{
		// U75 - 010610 - lee note : realised that the path SHOULD be used, and should not expect the file to be local!
		// when creating zip files, collect the relative paths then sit in the root folder and build the zip file..
		strrchr( lpszFilePath, '\\' );
		if ( !szFilename ) szFilename = strrchr( lpszFilePath, '/' );
		if ( !szFilename ) szFilename = lpszFilePath;
		else szFilename++;
	}
	if (!OpenNewFile(header, iLevel, szFilename)) return false;
	...
	if (!IsDirectory(header.m_uExternalAttr))
	{
		...
		BOOL bRet = f.Open(szFilename, CFile::modeRead | CFile::shareDenyWrite, e);
		e->Delete();
		if (!bRet)
			return false;
