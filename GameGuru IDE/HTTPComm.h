...
public:
	...
	bool	Login2( LPCSTR szUsername, LPCSTR szPassword );
	void	Logout( );
	void	RefreshSession( );
	...
	LPCSTR	UploadToClubCheck ( LPCSTR szKey, LPCSTR szDescription, LPCSTR szFilename, int iClubID=0 );
	LPCSTR	UploadToClub ( LPCSTR szKey, LPCSTR szDescription, LPCSTR szFilename, LPCSTR szThumbnail, int iClubID=0 );
	LPCSTR	GetClubLimits ( LPCSTR szKey, int iClubID=0 );
	LPCSTR	GetClubList ( LPCSTR szKey );
...
private:
	...
	bool HandleReply3( const char* szReply );
