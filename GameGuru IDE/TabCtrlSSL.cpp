...
BEGIN_MESSAGE_MAP(CTabCtrlSSL, CTabCtrl)
	...
	ON_NOTIFY_REFLECT (NM_KEYDOWN, OnSelChange)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CTabCtrlSSL::OnKeyDown (NMHDR* pNMHDR, LRESULT* pResult)
{
	int c = 0;
}
...
int CTabCtrlSSL::AddPage (LPCTSTR pszTitle, int nPageID, TabDelete tabDelete) {
	...
	GetClientRect (&rect);
	ResizeDialog (nIndex, rect.Width (), rect.Height ());
	//this->SetItemSize( CSize(30,30) );
	//this->SetPadding( CSize(0,10) );
