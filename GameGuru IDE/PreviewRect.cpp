...
int CPreviewRect::GetZoomedWidth( )
{
	...
	// get bitmap information
	if (m_visible->GetObject(sizeof(bmpInfo), &bmpInfo) == 0)
	{
		return 0;
	}
...
int CPreviewRect::GetZoomedHeight( )
{
	if ( !m_visible ) return 0;
...
void CPreviewRect::StartDragging( int x, int y )
{
	ptStart.x = x;
	ptStart.y = y;
...
void CPreviewRect::ContinueDragging( int x2, int y2 )
{
	ptEnd.x = x2;
	ptEnd.y = y2;
...
CBitmap* CPreviewRect::EndDragging( )
{
	if ( !m_bDragging ) return NULL;
	m_bDragging = false;
	...
	int iWidth = ptEnd.x - ptStart.x;
	int iHeight = ptEnd.y - ptStart.y;

	if ( iWidth < 2 || iHeight < 2 ) return NULL;

	CDC* pDC = GetDC( );
	CDC *pOrigDC = new CDC;
	CDC *pSelDC = new CDC;

	pOrigDC->CreateCompatibleDC( pDC );
	pSelDC->CreateCompatibleDC( pDC );

	CBitmap *pSelection = new CBitmap;
	pSelection->CreateCompatibleBitmap(pDC, iWidth, iHeight);

	ReleaseDC( pDC );

	CBitmap *pOldBmp1 = pOrigDC->SelectObject( m_visible );
	CBitmap *pOldBmp2 = pSelDC->SelectObject( pSelection );

	pSelDC->BitBlt( 0,0, iWidth, iHeight, pOrigDC, ptStart.x, ptStart.y, SRCCOPY );

	pOrigDC->SelectObject( pOldBmp1 );
	pSelDC->SelectObject( pOldBmp2 );
...
void CPreviewRect::CancelDragging( )
{
	m_bDragging = false;
}
...
void CPreviewRect::doDrawBackground()
{
	...
	if ( m_bDragging && m_bitmap )
	{
		CPen pen( PS_DASH, 1, RGB(0,0,0) );
		CPen *oldPen = pDC->SelectObject( &pen );

		pDC->MoveTo( ptStart.x, ptStart.y );
		pDC->LineTo( ptEnd.x, ptStart.y );
		pDC->LineTo( ptEnd.x, ptEnd.y );
		pDC->LineTo( ptStart.x, ptEnd.y );
