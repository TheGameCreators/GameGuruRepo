...
void cBuildGameLevel::Update ( cIPC* pIPC )
{
	...
	{
		// arena mode cannot edit FPI scripts (as they are geared for single game templates only)
		m_MapNumbers.EnableWindow ( FALSE );
		...
		// disable list of levels as we only have one level for the arena
	...
	{
		// restore buttons that may have been disabled by above
		m_MapNumbers.EnableWindow ( TRUE );
