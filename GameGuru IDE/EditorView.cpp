...
void CEditorView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	// 051013 - stops double mouse click DING sound!
...
void CEditorView::OnMouseMove ( UINT nFlags, CPoint point )
{
	...
	/* lee - 131014 - not at all sure why we need to call enumwindows inside mousemove?! (and EnumWindowsProc1 does not seem to do anything real)
		// 100105
...
{
	/* moved to contstant loop for better responsiveness
...
void CEditorView::OnTimer(UINT nIDEvent) 
{
	...
	// detect keys not covered by OnKeyDown calls
	DWORD dwAsyncValueA = ::GetAsyncKeyState (VK_ADD);
	if ( dwAsyncValueA & 0x8000 ) theApp.SetFileMapData ( 100, 1001 );
	DWORD dwAsyncValueS = ::GetAsyncKeyState (VK_SUBTRACT);
	if ( dwAsyncValueS & 0x8000 ) theApp.SetFileMapData ( 100, 1002 );
