...
local function processRotation( e, p, plQ )
	...
	if promptsOn then Prompt( "Use mouse to rotate " .. names[ e ] ) end		
...
function pickuppable_main(e)
	...
	if max( dims.w, dims.h, dims.l ) > maxSize then
		if promptsOn then Prompt( names[ e ] .. " is too large to carry!" ) end
		if handShowing == 'hand1' then
			HideHand( handShowing )
			ShowHand( 'hand2' )
		end
	...
	elseif dims.m > maxWeight then
		if promptsOn then Prompt( names[ e ] .. " is too heavy to lift!" ) end
		if handShowing == 'hand1' then
			HideHand( handShowing )
			ShowHand( 'hand2' )
		end
		...
		if g_MouseClick == 1 then
			...
			if currWeight > maxWeight then
				if promptsOn then 
					PromptDuration( "Stack is too heavy, remove some items or get stronger!", 2000 )
				end
				if handShowing == 'hand1' then
					HideHand( handShowing )
					ShowHand( 'hand2' )
