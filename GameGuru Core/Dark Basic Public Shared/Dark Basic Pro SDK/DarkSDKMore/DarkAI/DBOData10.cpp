...
sMesh::sMesh ( )
{
	...
	PrimitiveType			= D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	iDrawVertexCount		= 0;
	iDrawPrimitives			= 0;
	...
	// set default alpha as 1.0f (full)
	fAlphaFactor			= 1.0f;
}
...
sMesh::~sMesh ( )
{
	...
	// release all resources used by mesh
	SAFE_RELEASE ( pVertexBuffer );
	SAFE_RELEASE ( pIndexBuffer );
	SAFE_RELEASE ( pVertexLayout );
	SAFE_RELEASE ( pVertexStreamBuffer );
	SAFE_RELEASE ( pVertexStreamLayout );
	SAFE_RELEASE ( pIndexAdjBuffer );

	SAFE_DELETE ( pInstanceData );
	SAFE_DELETE ( pInstanceTextureList );
	SAFE_RELEASE ( pInstanceDataBuffer );
}
...
sObject::sObject ( )
{
	...
	bInstanceParent			= false;
	pInstanceOfObject		= NULL;
	iInstanceIndex			= 0;
	fInstanceNonBoneAnimFrame = -1.0f;
	pmatInstanceNonBoneCombined = NULL;
	...
	bSoftUsingDepthRead		= false;			// x10 - 190307 - used to record if object requires the reading of the depth buffer
	...
	dwCameraMaskBits		= 0xFFFFFFFF;		// by default, all objects render to all cameras
	dwShadowCameraMaskBits	= 0xFFFFFFFF;		// by default, all object shadows render to all cameras
	dwCubeMapCameraMaskBits = 0xFFFFFFFF;		// by default, all objects will be rendered in cubemaps
	dwCubeMapRequiredFromCameraID = 0;			// object does not act as center of a cube map render
}
...
sObject::~sObject ( )
{
	...
	SAFE_DELETE			( pmatInstanceNonBoneCombined );
