...
class cSpecialEffect
{
	...
	public:
		...
		ID3D10EffectTechnique*	m_pEffectTechniqueSO;
...
//		char				m_pEffectName [ MAX_STRING ];
		...
		bool				m_bTechniqueBasedOnTextureCount;						
...
struct sTexture
{
	...
	ID3D10ShaderResourceView*		pTexturesRef;								// reference to image texture
	ID3D10ShaderResourceView*		pCubeTexture;								// cube texture reference
	...
	// reserved members
	DWORD							dwTextureArrayMode;							// 200307 - set to 1 if texture array applied to this stage
...
/* obsolete as manager completely replaced
struct sDrawBuffer
{
	// draw buffer data
...
struct sMeshDraw
{
	...
	//DX10 WORD*					pIndices;									// index array
	DWORD*							pIndices;									// index array
	DWORD							dwVertexCount;								// number of vertices
	DWORD							dwIndexCount;								// number of indices
	D3D10_PRIMITIVE_TOPOLOGY		PrimitiveType;								// primitive type
	int								iDrawVertexCount;							// number of vertices to be used when drawing
	int								iDrawPrimitives;							// number of actual primitives to draw

	ID3D10InputLayout*				pVertexLayout;								// Replaced sDrawBuffer entirely
	ID3D10Buffer*					pVertexBuffer;
	ID3D10Buffer*					pIndexBuffer;
	ID3D10Buffer*					pVertexStreamBuffer;						// used for stream-out data (skinning technique)
	ID3D10InputLayout*				pVertexStreamLayout;						// used to define the format of the streamed-out vertex data
	ID3D10Buffer*					pIndexAdjBuffer;							// used to hold adjacency data (shadow technique)
	bool							bSkinOncePerSync;

	DWORD							dwInstanceDataCount;
	sMeshInstanceData*				pInstanceData;
	ID3D10ShaderResourceView*		pInstanceTextureList;
	ID3D10Buffer*					pInstanceDataBuffer;
	bool							bUpdateInstanceData;
...
//	DWORD							dwAlphaOverride;							// tfactor alpha override (true object transparency)
	float							fAlphaFactor;								// alpha factor override (true object transparency)
...
{
	// object maintainance
	sObject*						pParentObject;								// used after meshes sorted into effect order

	// OLD object maintainance and management
					   ...
					   sFramePosition
{
	...
	// reserved members
	DWORD							dwReservedF1;								// reserved - maintain plugin compat.
...
struct sPositionProperties
{
	...
	bool							bIgnoreOrigMatrix;							// flag to ignore the original matrix carried in from X files (must ignore FPSC X9 characters as they have bone matrices which also contain original data)
...
{
	// x10 properties
	DWORD							dwCameraMaskBits;							// masks object to certain cameras
	DWORD							dwShadowCameraMaskBits;						// masks object shadows to certain cameras
	DWORD							dwCubeMapCameraMaskBits;					// masks object cubemap to certain cameras
	DWORD							dwCubeMapRequiredFromCameraID;				// specifies camid+1 used as cubemapsource
	bool							bUpdateCubeMapOncePerCycle;					// reset each cycle (so multiple cubemap renders are avoided)
	...
	// reserved members
	bool							bSoftUsingDepthRead;						// set to true if object requires the reading of the depth buffer
	...
	DWORD							dwReservedOP2;								// reserved - maintain plugin compat.
...
{
	// instance per object
	bool							bInstanceParent;							// is a host to instances
	sObject*						pInstanceOfObject;							// ref to parent object
	int								iInstanceIndex;								// place in instancedata list
	float							fInstanceNonBoneAnimFrame;					// for anim that transforms matCombined
	D3DXMATRIX*						pmatInstanceNonBoneCombined;				// for anim that transforms matCombined
