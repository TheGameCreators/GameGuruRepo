...
void SpinningCubeRenderer::PositionHologram(SpatialPointerPose const& pointerPose)
{
	...
	{
        const float3 headPosition = float3(0,0,0);
        const float3 headDirection = float3(0,0,1);
        constexpr float distanceFromUser = 2.0f; // meters
        const float3 gazeAtTwoMeters = headPosition + (distanceFromUser * headDirection);
        SetPosition(gazeAtTwoMeters);
	}
...
// target array index.
void SpinningCubeRenderer::Render(void* pLeftCamTex, void* pRightCamTex)
{
	...
	// Feed textures in so I can see my stereo camera texture in WinRT render (a start!)
	ID3D11ShaderResourceView* lpTexture = (ID3D11ShaderResourceView*)pLeftCamTex;
	context->PSSetShaderResources ( 0, 1, &lpTexture );

	// Sampler too
	if ( m_pSamplerLinear == NULL )
	{
		ID3D11Device4* pD3D = m_deviceResources->GetD3DDevice();
		...
		pD3D->CreateSamplerState ( &sampDesc, &m_pSamplerLinear);
	}
	context->PSSetSamplers ( 0, 1, &m_pSamplerLinear );
...
std::future<void> SpinningCubeRenderer::CreateDeviceDependentResources()
{
    ...
    static const std::array<VertexPositionColor, 8> cubeVertices =
        { {
            { XMFLOAT3(-0.1f, -0.1f, -0.1f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT2(0.0f, 0.0f) },
            { XMFLOAT3(-0.1f, -0.1f,  0.1f), XMFLOAT3(0.0f, 0.0f, 1.0f), XMFLOAT2(0.0f, 0.0f) },
            { XMFLOAT3(-0.1f,  0.1f, -0.1f), XMFLOAT3(0.0f, 1.0f, 0.0f), XMFLOAT2(0.0f, 1.0f) },
            { XMFLOAT3(-0.1f,  0.1f,  0.1f), XMFLOAT3(0.0f, 1.0f, 1.0f), XMFLOAT2(0.0f, 1.0f) },
            { XMFLOAT3( 0.1f, -0.1f, -0.1f), XMFLOAT3(1.0f, 0.0f, 0.0f), XMFLOAT2(1.0f, 0.0f) },
            { XMFLOAT3( 0.1f, -0.1f,  0.1f), XMFLOAT3(1.0f, 0.0f, 1.0f), XMFLOAT2(1.0f, 0.0f) },
            { XMFLOAT3( 0.1f,  0.1f, -0.1f), XMFLOAT3(1.0f, 1.0f, 0.0f), XMFLOAT2(1.0f, 1.0f) },
            { XMFLOAT3( 0.1f,  0.1f,  0.1f), XMFLOAT3(1.0f, 1.0f, 1.0f), XMFLOAT2(1.0f, 1.0f) },
        } };
