...
void LMPoly::SetDiffuseUV( Point *v1, Point *v2, Point *v3 )
{
	...
	diffuseuv3[0] = v3->x; diffuseuv3[1] = v3->y; 
}
...
void LMPoly::Flatten( )
{
	float fMax = (float)fabs( normal[0] );
	iUIndex = 2;
	iVIndex = 1;

	if ( fabs( normal[1] ) > fMax ) 
	{ 
		fMax = (float)fabs( normal[1] );
		iUIndex = 0;
		iVIndex = 2;
	}
	if ( fabs( normal[2] ) > fMax ) 
	{ 
		fMax = (float)fabs( normal[2] );
		iUIndex = 0;
		iVIndex = 1;
	}
...
void LMPoly::CalculateArea( )
{
	...
	fArea = ((float)sqrt (fDistT) * (float)sqrt (fDist1) ) / 2.0f;
}
...
bool LMPoly::Joined( LMPoly* pOtherPoly )
{
	...
	float fDiffX = (float)fabs( vert1[0] - pOtherPoly->vert1[0] );
	float fDiffY = (float)fabs( vert1[1] - pOtherPoly->vert1[1] );
	float fDiffZ = (float)fabs( vert1[2] - pOtherPoly->vert1[2] );
	if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
	else
	{
		fDiffX = (float)fabs( vert1[0] - pOtherPoly->vert2[0] );
		fDiffY = (float)fabs( vert1[1] - pOtherPoly->vert2[1] );
		fDiffZ = (float)fabs( vert1[2] - pOtherPoly->vert2[2] );
		if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
		else
		{
			fDiffX = (float)fabs( vert1[0] - pOtherPoly->vert3[0] );
			fDiffY = (float)fabs( vert1[1] - pOtherPoly->vert3[1] );
			fDiffZ = (float)fabs( vert1[2] - pOtherPoly->vert3[2] );
			if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
		}
	}

	fDiffX = (float)fabs( vert2[0] - pOtherPoly->vert1[0] );
	fDiffY = (float)fabs( vert2[1] - pOtherPoly->vert1[1] );
	fDiffZ = (float)fabs( vert2[2] - pOtherPoly->vert1[2] );
	if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
	else
	{
		fDiffX = (float)fabs( vert2[0] - pOtherPoly->vert2[0] );
		fDiffY = (float)( vert2[1] - pOtherPoly->vert2[1] );
		fDiffZ = (float)fabs( vert2[2] - pOtherPoly->vert2[2] );
		if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
		else
		{
			fDiffX = (float)fabs( vert2[0] - pOtherPoly->vert3[0] );
			fDiffY = (float)fabs( vert2[1] - pOtherPoly->vert3[1] );
			fDiffZ = (float)fabs( vert2[2] - pOtherPoly->vert3[2] );
			if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
		}
	}

	fDiffX = (float)fabs( vert3[0] - pOtherPoly->vert1[0] );
	fDiffY = (float)fabs( vert3[1] - pOtherPoly->vert1[1] );
	fDiffZ = (float)fabs( vert3[2] - pOtherPoly->vert1[2] );
	if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
	else
	{
		fDiffX = (float)fabs( vert3[0] - pOtherPoly->vert2[0] );
		fDiffY = (float)fabs( vert3[1] - pOtherPoly->vert2[1] );
		fDiffZ = (float)fabs( vert3[2] - pOtherPoly->vert2[2] );
		if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
		else
		{
			fDiffX = (float)fabs( vert3[0] - pOtherPoly->vert3[0] );
			fDiffY = (float)fabs( vert3[1] - pOtherPoly->vert3[1] );
			fDiffZ = (float)fabs( vert3[2] - pOtherPoly->vert3[2] );
			if ( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ < fMinDist ) iCount++;
		}
	}
...
void LMPoly::CalculateTexUV( int iStartU, int iStartV, float fMinU2, float fMinV2, int iTexSizeU, int iTexSizeV )
{
	...
	fU1 = (fDiffU + iStartU + 0.0f) / iTexSizeU;
	fV1 = (fDiffV + iStartV + 0.0f) / iTexSizeV;

	fDiffU = (vert2[iUIndex] - fMinU2) / fQuality;
	fDiffV = (vert2[iVIndex] - fMinV2) / fQuality;

	fU2 = (fDiffU + iStartU + 0.0f) / iTexSizeU;
	fV2 = (fDiffV + iStartV + 0.0f) / iTexSizeV;

	fDiffU = (vert3[iUIndex] - fMinU2) / fQuality;
	fDiffV = (vert3[iVIndex] - fMinV2) / fQuality;

	fU3 = (fDiffU + iStartU + 0.0f) / iTexSizeU;
	fV3 = (fDiffV + iStartV + 0.0f) / iTexSizeV;
}
...
void LMCurvedPoly::GetNormal( float fPosX, float fPosY, float fPosZ, float *fNormX, float *fNormY, float *fNormZ )
{
	...
	fDist1 = (float)sqrt( fNormalBCX*fNormalBCX + fNormalBCY*fNormalBCY + fNormalBCZ*fNormalBCZ );
	fNormalBCX /= fDist1;
	fNormalBCY /= fDist1;
	fNormalBCZ /= fDist1;
	...
	fDist1 = (float)sqrt( fDiffX*fDiffX + fDiffY*fDiffY + fDiffZ*fDiffZ );
