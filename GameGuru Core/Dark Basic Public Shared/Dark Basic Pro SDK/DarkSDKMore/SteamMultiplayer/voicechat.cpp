	...
	if ( m_bIsActive )
	{
		...
		{	
			...
			if ( res == k_EVoiceResultOK && nBytesWritten > 0 )
			{
				...
				m_ulLastTimeTalked = (uint64)GetCounterPassedTotal();
				...
				if ( m_hVoiceLoopback !=  0 )
				{
					...
					{
						AddVoiceData( m_hVoiceLoopback, m_ubUncompressedVoice, numUncompressedBytes );
					}
				}
			}
		}		
...
bool CVoiceChat::HandleMessage( CSteamID fromSteamID, EMessage eMsg, const void *pMessage )
{
	...
	if ( iter != m_MapConnections.end() )
	{
		...
		if ( eMsg == k_EMsgVoiceChatData )
		{
			...
			chatClient.ulLastReceiveVoiceTime = (uint64)GetCounterPassedTotal();
			...
			if ( res == k_EVoiceResultOK && numUncompressedBytes > 0 )
			{
				...
				{
					chatClient.hVoiceChannel = HCreateVoiceChannel();
				}

				AddVoiceData( chatClient.hVoiceChannel, pbUncompressedVoice, numUncompressedBytes );
			}
		}
	}
...
bool CVoiceChat::IsPlayerTalking( CSteamID steamID )
{
	...
	{
		// thats ourself
		if ( m_ulLastTimeTalked + 250 > GetCounterPassedTotal() )
			return true;
	}
	else
	{
		...
		if ( iter != m_MapConnections.end() )
		{
			...
			{
				if ( (iter->second.ulLastReceiveVoiceTime + 250) >  GetCounterPassedTotal() )
				{
...
void CVoiceChat::StopVoiceChat()
{
	if ( m_bIsActive )
	{
		...
		for( iter = m_MapConnections.begin(); iter != m_MapConnections.end(); ++iter )
		{
			...
			DestroyVoiceChannel( iter->second.hVoiceChannel );
		...
		if ( m_hVoiceLoopback )
		{
			DestroyVoiceChannel( m_hVoiceLoopback );
			m_hVoiceLoopback = 0;
		}
...
{
	const uint64 ulTimeNow = (uint64)GetCounterPassedTotal();
	const uint64 ulTimeoutLimit = (uint64)(10 * 1000); // 10 seconds
	// 
	std::map< uint64, VoiceChatConnection_t >::iterator iter;
	for( iter = m_MapConnections.begin(); iter != m_MapConnections.end(); ++iter )
	{
