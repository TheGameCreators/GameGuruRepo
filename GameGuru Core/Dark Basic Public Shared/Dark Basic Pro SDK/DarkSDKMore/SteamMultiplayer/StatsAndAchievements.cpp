...
void CStatsAndAchievements::OnGameStateChange( EClientGameState eNewState )
{
	...
	case k_EClientGameActive:
		...
		m_flGameFeetTraveled = 0;
		m_ulTickCountGameStart = (uint64)GetCounterPassedTotal();
		break;
	case k_EClientFindInternetServers:
		break;	
	case k_EClientGameWinner:
		if ( Client()->BLocalPlayerWonLastGame() )
			m_nTotalNumWins++;
		else
			m_nTotalNumLosses++;
		// fall through
	case k_EClientGameDraw:
		...
		// Calc game duration
		m_flGameDurationSeconds = ( GetCounterPassedTotal() - m_ulTickCountGameStart ) / 1000.0; // dave
...
void CStatsAndAchievements::Render()
{
	...
	if ( !m_bStatsValid )
	{
		...
		m_pGameEngine->BDrawString( m_hDisplayFont, rect, GGCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_VCENTER, rgchBuffer );
		...
		m_pGameEngine->BDrawString( m_hDisplayFont, rect, GGCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_TOP, rgchBuffer );
...
void CStatsAndAchievements::DrawAchievementInfo( RECT &rect, Achievement_t &ach )
{
	...
	m_pGameEngine->BDrawString( m_hDisplayFont, rect2, GGCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_LEFT|TEXTPOS_VCENTER, rgchBuffer );
	*/
}
...
void CStatsAndAchievements::DrawStatInfo( RECT &rect, const char *pchName, float flValue )
{
	...
	m_pGameEngine->BDrawString( m_hDisplayFont, rect, GGCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_LEFT|TEXTPOS_VCENTER, rgchBuffer );
	*/
}
