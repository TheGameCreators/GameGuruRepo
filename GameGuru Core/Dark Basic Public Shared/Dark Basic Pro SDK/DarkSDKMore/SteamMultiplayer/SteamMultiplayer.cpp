...
public:
	...
	bool isItemCreated = 0;
	int isItemDownloaded = 0;
	int isItemUploaded = 0;
	int isItemSubscribed = 0;
...
void CSteamUserGeneratedWorkshopItem::OnWorkshopItemDownloadDone( DownloadItemResult_t  *pCallback )
{
	if ( pCallback->m_nPublishedFileId == WorkshopItemToDownloadID )
	{
		...
		{
			// LB: Seems to be hitting here with 'k_EResultFail' - generic failure!!!
			// Found this, might relate to restricted accounts: https://help.steampowered.com/en/faqs/view/71D3-35C2-AD96-AA3A
			//char pNum[32];
			//sprintf(pNum, "%d", (int)pCallback->m_eResult);
			//MessageBox (NULL, "OnWorkshopItemDownloadDone", pNum, MB_OK);
			isItemDownloaded = -1;
			return;
		}

		// LB: It appears this was missing for the longest time - needed to locate the workshop files that have been downloaded!
		// maybe it was accidentally removed during the C++ port or STEAM/PHOTON update
		uint32 timestamp;
