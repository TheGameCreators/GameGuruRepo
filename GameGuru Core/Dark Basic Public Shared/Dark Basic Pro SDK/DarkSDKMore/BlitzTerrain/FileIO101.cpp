...
//Loop through LODLevels
	...
	//Loop through sectors
		...
		//Insert Sector header into Sector
			...
			SectorPtr->Pos_x=float(SectorPtr->Column*LODLevelPtr->SectorSize+0.5*LODLevelPtr->SectorSize);
			SectorPtr->Pos_y=0.0f;
			SectorPtr->Pos_z=float(SectorPtr->Row*LODLevelPtr->SectorSize+0.5*LODLevelPtr->SectorSize);
		...
		//Create QuadMap
			...
			//Create quadmap
				...
				SectorPtr->QuadMap->GenerateFromBuffer101(QuadMapBuffer,QuadMapBufferSize,SectorPtr);
				free(QuadMapBuffer);
			}

		//Clear Sector Header
			BT_Intern_FillSectorHeaderDefault101(SectorHeader);
		}

	//Clear LODLevel Header
		BT_Intern_FillLODLevelHeaderDefault101(LODLevelHeader);
	}
...
{
//Make Header
	s_BT_FileTerrainHeader101* Header=(s_BT_FileTerrainHeader101*)malloc(sizeof(s_BT_FileTerrainHeader101));
	if(Header==nullptr)
		BT_Intern_Error(C_BT_ERROR_MEMORYERROR);

//Set default values
	BT_Intern_FillTerrainHeaderDefault101(Header);
...
{
//Make Header
	s_BT_FileLODLevelHeader101* Header=(s_BT_FileLODLevelHeader101*)malloc(sizeof(s_BT_FileLODLevelHeader101));
	if(Header==nullptr)
		BT_Intern_Error(C_BT_ERROR_MEMORYERROR);

//Set default values
	BT_Intern_FillLODLevelHeaderDefault101(Header);
...
{
//Make Header
	s_BT_FileSectorHeader101* Header=(s_BT_FileSectorHeader101*)malloc(sizeof(s_BT_FileSectorHeader101));
	if(Header==nullptr)
		BT_Intern_Error(C_BT_ERROR_MEMORYERROR);

//Set default values
	BT_Intern_FillSectorHeaderDefault101(Header);
