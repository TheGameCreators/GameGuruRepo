 ...
 void MakeVegPatch(int iVegObj, float fVegHeight, float fVegWidth, int iX, int iZ, int iOptionalSkipGrassMemblock)
{
	...
	// 231017 - if no grass within this segment, can create dummy and skip quickly
	/* this will not work as grass is ROLLING which means its reused as you move around terrain!!
	if ( iOptionalSkipGrassMemblock > 0 )
	{
		// in game exe, less grass than needed for editor use
		bWeHaveGrassHere = false;
		#define MAXTEXTURESIZECOPY 2048
		int tvegareax1 = iX*fVegWidth;
		int tvegareax2 = (iX*fVegWidth)+fVegWidth;
		int tvegareaz1 = iZ*fVegHeight;
		int tvegareaz2 = (iZ*fVegHeight)+fVegHeight;
		if ( tvegareax1 < 0 )  tvegareax1 = 0;
		if ( tvegareax2 > MAXTEXTURESIZECOPY-1 ) tvegareax2 = MAXTEXTURESIZECOPY - 1;
		if ( tvegareaz1 < 0 )  tvegareaz1 = 0;
		if ( tvegareaz2 > MAXTEXTURESIZECOPY-1 ) tvegareaz2 = MAXTEXTURESIZECOPY - 1;
		for ( int tvz = tvegareaz1; tvz <= tvegareaz2; tvz++ )
		{
			int tZStep = tvz*MAXTEXTURESIZECOPY;
			for ( int tvx = tvegareax1; tvx <= tvegareax2; tvx++ )
			{
				unsigned char grassvalue = ReadMemblockByte ( iOptionalSkipGrassMemblock, (4+4+4+((tvx+tZStep)*4))+2 );
				if ( grassvalue > 0 ) { bWeHaveGrassHere = true; break; }
			}
			if ( bWeHaveGrassHere == true) break;
		}
	}
	*/
	...
	if ( bWeHaveGrassHere == true )
	{
		if ( iVegObj == iGridObjectStart )
		{
			...
			MakeObject(iVegObj,iGrassObj,0);
			for (int iGrass = 1; iGrass < iVegPerMesh + 1; iGrass++)
			{
				...
				AddLimb(iVegObj,iGrass,iGrassObj);
 ...
 void UpdateVegPatch(int iVegObj, int iTerrainID, float fVegX, float fVegZ)
{
	...
	for (int iGrass=0; iGrass < iVegPerMesh + 1; iGrass++){
		...
		fNewHigh = fFloor + fHigh - fLow;
					
		// Now reposition all of our vertices for this grass clump					
		for (int fGrassVert = 0; fGrassVert < iGrassClumpVerts; fGrassVert++){
			fVertX = (GetVertexDataPositionX(iVertexCount));
			fVertY = (GetVertexDataPositionY(iVertexCount));
			fVertZ = (GetVertexDataPositionZ(iVertexCount));
			if (abs(fVertY - fLow) < 0.01f){
				SetVertexDataPosition(iVertexCount,fVertX,fFloor,fVertZ);
			}else{
				SetVertexDataPosition(iVertexCount,fVertX,fNewHigh,fVertZ);
			}				
			iVertexCount++;
		}					
	}
	...
	for (int iX = iLeftEdge; iX < iRightEdge + 1; iX++){		
		for (int iZ = iBackEdge; iZ < iFrontEdge + 1; iZ++){
			...
			if (bGridExist[iX][iZ]){
				...
				if (ObjectExist(iVegObj) == 1){	// Safety check
		...
		for (int iZ = iBackEdge; iZ < iFrontEdge + 1; iZ++){
			...
			if (ObjectExist(iVegObj) == 1){ // Safety check
				...
				if (bGridExist[iX][iZ]){
					...
					// Apply dynamic shadow image to all veg objects
					//TextureObject(iVegObj,2,iDynamicShadowImage);			// not used any more		
				}else{
					HideObject(iVegObj);
				}
			}
	...
	for (int iObj = iGridObjectStart; iObj < iGridObjectEnd + 1; iObj++)
	{
		if (ObjectExist(iObj) == 1)
		{
			...
			else
			{
				...
				//PE: g not here, ibr not used in veg anyway.
				//if (g.memskipibr == 0) {
				//	TextureObject(iObj, 8, iPBRCurveImg);//GlossCurveMap
				//}
			}
		}
	}	
