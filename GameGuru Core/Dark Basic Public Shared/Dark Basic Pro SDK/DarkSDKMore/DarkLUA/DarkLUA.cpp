...
// Externs that link with the DBCore calls to control sprite draw order
 ...
 */
 ...
 int GetEntityData ( lua_State *L, int iDataMode )
 {
	...
	if ( iEntityIndex > 0 )
	{
		...
		else
		{
			...
			switch ( iDataMode )
			{
				//case 101 : iReturnValue = t.entityelement[iEntityIndex].eleprof.??integer??; break;
			}
 ...
 int AIEntityStop(lua_State *L)
 {
	...
	//SteamSendLua ( 21 , lua_tointeger(L, 1), 0 );
...
int AIGetVisualSetting ( lua_State *L, int iMode )
{
	...
	switch ( iMode )
	{
		...
		case 9 : lua_pushnumber ( L, t.visuals.AmbienceGeen_f ); break;
...
int SetGamePlayerControlData ( lua_State *L, int iDataMode )
{
	...
	switch ( iDataMode )
	{
		...
		case 14 : t.playercontrol.basespeed_f = lua_tonumber(L, 1); break;
...
int GetGamePlayerControlData ( lua_State *L, int iDataMode )
{
	...
	switch ( iDataMode )
	{
		...
		case 32 : lua_pushnumber ( L, t.playercontrol.wobblespeed_f ); break;
...
DARKLUA_API void LuaCallSilent()
{
 ...
 DARKLUA_API DWORD LuaGetString ( DWORD pOldString, LPSTR pString , int id )
 {
	...
	{
		//g_pGlob->CreateDeleteString ( (DWORD*)&pReturnString, 1 );
