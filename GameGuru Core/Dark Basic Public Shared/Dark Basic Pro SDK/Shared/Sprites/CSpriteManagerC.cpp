...
void CSpriteManager::RenderDrawList(tagSpriteData** pList, int iListSize, int iFilterMode)
{
	...
	if (g.FrameCount > 0) 
	{
		...
		for (int iTemp = 0; iTemp < iListSize; iTemp++)
		{
			...
			float fX = pCurrent->fX;
			float fY = pCurrent->fY;
			int iWidth = pCurrent->iWidth;
			int iHeight = pCurrent->iHeight;
			bool bVisible = pCurrent->bVisible;

			//PE: Looks like we can always use the lpVertices to draw stuff @Lee ?
			if (1 == 1 || fX < -1000) {
				fX = pCurrent->lpVertices[0].x;
				fY = pCurrent->lpVertices[0].y;
				iWidth = pCurrent->lpVertices[3].x - pCurrent->lpVertices[0].x;
				iHeight = pCurrent->lpVertices[3].y - pCurrent->lpVertices[0].y;
				UV_Min = ImVec2(pCurrent->lpVertices[0].tu, pCurrent->lpVertices[0].tv);
				UV_Max = ImVec2(pCurrent->lpVertices[3].tu, pCurrent->lpVertices[3].tv);
			}
			if (bVisible)
			{
