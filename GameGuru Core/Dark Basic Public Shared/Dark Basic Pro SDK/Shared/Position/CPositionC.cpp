...
extern void GetCullDataFromModel(int);
...
void TEXTGetAngleFromPoint(float x1, float y1, float z1, float x2, float y2, float z2, float* ax, float* ay, float* az)
{
	D3DXVECTOR3 Vector;
	Vector.x = x2-x1;
	Vector.y = y2-y1;
...
void Point ( int iID, float fX, float fY, float fZ )
{
	...
	*/
	...
	TEXTGetAngleFromPoint ( m_pPos->vecPosition.x, m_pPos->vecPosition.y, m_pPos->vecPosition.z,
