	...
	else
	{
		...
		if ( bEffectRendering )
		{
			...
			if ( pArtFlags )
			{
				...
				float fParallaxStrength = pMesh->fParallaxStrength;
				GGVECTOR4 vec4 = GGVECTOR4 ( fInvertNormal, fGenerateTangents, fBoostIntensity, fParallaxStrength);
...
bool CObjectManager::DrawMesh ( sMesh* pMesh, bool bIgnoreOwnMeshVisibility, sObject* pObject, sFrame* pFrame)
{
	...
	if (g_pGlob->dwRenderCameraID >= 31) {
		...
		if (pMesh->bCull == false) {
			//m_pImmediateContext->RSSetState(m_pRasterStateNoCull);
			m_pImmediateContext->RSSetState(m_pRasterStateNoCullShadow);
		}
		else {
			//m_pImmediateContext->RSSetState(m_pRasterState);
			//PE: Render backfaces only. to prevent shadow artifacts.
			m_pImmediateContext->RSSetState(m_pRasterStateShadow);
		}
	}
	else {
...
//g_ObjectList [ iDestinationID ]->collision.vecCentre
//PE: calculate distance from object to camera, dont use position but real object center.
float CObjectManager::CalculateObjectCenterDistanceFromCamera(sObject* pObject)
{
	...
	//PE: Use center of object on decals ... for better distance calc to transparent objects.
	float fdx = ( pObject->position.vecPosition.x + pObject->collision.vecColCenter.x ) - vecCamPos.x;
	float fdy = ( pObject->position.vecPosition.y + pObject->collision.vecColCenter.y ) - vecCamPos.y;
	float fdz = ( pObject->position.vecPosition.z + pObject->collision.vecColCenter.z ) - vecCamPos.z;

	//PE: See pivot.
	/*
	if (pObject->dwObjectNumber >= 70000)
	{
		if (ObjectExist(pObject->dwObjectNumber + 10000) == 0)
		{
			//MakeObjectBox(pObject->dwObjectNumber + 10000, 25.0, 25.0,25.0);
			MakeObjectSphere(pObject->dwObjectNumber + 10000, 10.0f);
			SetObjectMask(pObject->dwObjectNumber + 10000, 1);
			SetObjectCollisionOff(pObject->dwObjectNumber + 10000);
			DisableObjectZRead(pObject->dwObjectNumber + 10000);
			PositionObject(pObject->dwObjectNumber + 10000, pObject->position.vecPosition.x, pObject->position.vecPosition.y, pObject->position.vecPosition.z);
			extern int gguishadereffectindex;
			SetObjectEffect(pObject->dwObjectNumber + 10000, gguishadereffectindex);
			SetObjectEmissive(pObject->dwObjectNumber + 10000, (DWORD)(((255) & 0xff) << 24) | (((255) & 0xff) << 16) | (((196) & 0xff) << 8) | ((25) & 0xff));
			ShowObject(pObject->dwObjectNumber + 10000);
		}
		else {
			//PositionObject(pObject->dwObjectNumber + 10000, pObject->position.vecPosition.x, pObject->position.vecPosition.y, pObject->position.vecPosition.z);
			//PE: We need rotation to calculate the object center position. (also need col*scale).
			//
			PositionObject(pObject->dwObjectNumber + 10000, (pObject->position.vecPosition.x + pObject->collision.vecColCenter.x), (pObject->position.vecPosition.y + pObject->collision.vecColCenter.y), (pObject->position.vecPosition.z + pObject->collision.vecColCenter.z));
			//PositionObject(pObject->dwObjectNumber + 10000, (pObject->position.vecPosition.x + pObject->collision.vecCentre.x), (pObject->position.vecPosition.y + pObject->collision.vecCentre.y), (pObject->position.vecPosition.z + pObject->collision.vecCentre.z));
			ShowObject(pObject->dwObjectNumber + 10000);
		}
	}
	*/
