...
HRESULT CascadedShadowsManager::ReleaseAndAllocateNewShadowResources( LPD3DXEFFECT pEffectPtr )
{
        ...
        || m_CopyOfCascadeConfig.m_iBufferSize != m_pCascadeConfig->m_iBufferSize )
    {
		...
		DWORD dwSurfaceWidth = 1280;
		DWORD dwSurfaceHeight = 1280;
...
HRESULT CascadedShadowsManager::InitFrame ( LPD3DXEFFECT pEffectPtr  )
{
	...
	m_pViewerCamera->m_fFar = 7000.0f; // PE test from 5000 for depthmap4
	...
	vecAt.z *= 100.0f;
	vecAt += vecEye;
    // PE: If the other change dont fix the flicker , we can divede this to nearest 10 and round to nearest pixel.
    // this will limit the flickering. no need to move camera so often.
    ...
    for( INT iCascadeIndex=0; iCascadeIndex < m_CopyOfCascadeConfig.m_nCascadeLevels; ++iCascadeIndex ) 
    {
		...
		// Added this for Prebens experiment
        ...
        // PE: not sure if set must be used.
...
//        FLOAT fFarPlane = 5000.0f;
        // PE-test: dm4 dont think this is far enoff , perhaps thats why 4 is not useable.
        FLOAT fFarPlane = 7000.0f;
...
HRESULT CascadedShadowsManager::RenderShadowsForAllCascades ( LPD3DXEFFECT pEffectPtr ) 
{
    ...
    for( INT currentCascade=m_CopyOfCascadeConfig.m_nCascadeLevels-1; currentCascade >= 0; currentCascade-- ) 
    {
		...
		if ( bPrepareObjectsAndFrustum==true )
		{
			...
			//Dave Performance
			//Backup existing vis list first
			if ( !g_bInEditor )
			{
			...
			//End of backing up existing vis list
		...
		{
			LPDIRECT3DTEXTURE9 pDebugSee = GetImagePointer ( 59951+currentCascade );
			if ( pDebugSee )
			{
	...
	{
		//Dave Performance - put previous vis lists back
		SetupFrustum ( 0.0f );		
		if ( g_bInEditor )
		{
			m_ObjectManager.SortVisibilityList();
