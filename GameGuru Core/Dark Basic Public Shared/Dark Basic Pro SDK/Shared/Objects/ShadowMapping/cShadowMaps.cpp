...
extern int g_TerrainShadows;

// Shadowmap Values from GG (filled in using init below)
int g_globalsrealshadowsize[8];
float g_globalsrealshadowdistance;
int g_globalsflashlightshadows;
int g_globalsspeedshadows;

void InitShadowMapDetails(int rs0,int rs1,int rs2,int rs3,int rs4,int rs5,int rs6,int rs7,float dist,int flash,int speed)
{
	g_globalsrealshadowsize[0] = rs0;
	g_globalsrealshadowsize[1] = rs1;
	g_globalsrealshadowsize[2] = rs2;
	g_globalsrealshadowsize[3] = rs3;
	g_globalsrealshadowsize[4] = rs4;
	g_globalsrealshadowsize[5] = rs5;
	g_globalsrealshadowsize[6] = rs6;
	g_globalsrealshadowsize[7] = rs7;
	g_globalsrealshadowdistance = dist;
	g_globalsflashlightshadows = flash;
	g_globalsspeedshadows = speed;
}
...
HRESULT CascadedShadowsManager::ReleaseAndAllocateNewShadowResources( LPGGEFFECT pEffectPtr )
{
        ...
        || m_CopyOfCascadeConfig.m_iBufferSize != m_pCascadeConfig->m_iBufferSize )
    {
		...
		for ( int c = 0; c < m_CopyOfCascadeConfig.m_nCascadeLevels; c++ )
		{
			...
			if ( g_globalsrealshadowsize[c] > 0) {
				m_depthTexture[c]->createTexture(m_pD3D, g_globalsrealshadowsize[c], g_globalsrealshadowsize[c]);
			}
			else {
                                                        ...
                                                        GGVECTOR4* pvCornerPointsWorld ) 
{
    ...
    static const DBXMVECTORU32 vGrabY = {0x00000000,0xFFFFFFFF,0x00000000,0x00000000};
    static const DBXMVECTORU32 vGrabX = {0xFFFFFFFF,0x00000000,0x00000000,0x00000000};
...
HRESULT CascadedShadowsManager::InitFrame ( LPGGEFFECT pEffectPtr  )
{
	...
	m_pViewerCamera->m_fFar = g_globalsrealshadowdistance; //PE: 5000.0f;//3000.0f;
    ...
    for( INT iCascadeIndex=0; iCascadeIndex < m_CopyOfCascadeConfig.m_nCascadeLevels; ++iCascadeIndex ) 
    {
        ...
        if( m_eSelectedCascadesFit == FIT_TO_SCENE ) 
        {
			...
			if (g_globalsrealshadowsize[iCascadeIndex] > 0) fWorldUnitsPerTexel = fCascadeBound / (float)g_globalsrealshadowsize[iCascadeIndex];
...
// allows the terrain to render to the shadow map quickly
#ifndef NOSTEAMORVIDEO
extern int g_iTerrainIDForShadowMap;
extern bool g_bRenderTerrainForShadowMap;
#endif
	...
	#endif
	{
		...
		for( INT currentCascade=m_CopyOfCascadeConfig.m_nCascadeLevels-1; currentCascade >= 0; currentCascade-- ) 
		{
			...
			#endif
			{
				...
				// and also render the terrain (fixing the issue of floating shadows from high objects)
				#ifndef NOSTEAMORVIDEO
				g_bRenderTerrainForShadowMap = false;
				if (g_TerrainShadows == 1) g_bRenderTerrainForShadowMap = true;
				if (g_iTerrainIDForShadowMap > 0 && g_bRenderTerrainForShadowMap == true)
				{
					BT_SetCurrentCamera(0);
					BT_UpdateTerrainCull(g_iTerrainIDForShadowMap);
					BT_UpdateTerrainLOD(g_iTerrainIDForShadowMap);
					BT_RenderTerrain(g_iTerrainIDForShadowMap);
					BT_Intern_Render();
					g_bRenderTerrainForShadowMap = false;
					//Without this, terrain cascade0 (for other objects) will not work, when using a flat terrain.
					m_ObjectManager.ResetIBRef();
				}
				// restore render camera ID
            ...
            ) 
{
	...
	for(int index=0; index < m_CopyOfCascadeConfig.m_nCascadeLevels+g_globalsflashlightshadows; ++index )
	{
		if(g_globalsspeedshadows != 0)
		{
...
//	dynamic_sun_position(iEffectID);
	#else
