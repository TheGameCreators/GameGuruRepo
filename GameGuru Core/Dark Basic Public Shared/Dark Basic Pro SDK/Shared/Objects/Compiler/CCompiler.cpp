...
bool cCompiler::CompileScene ( sMesh* pMeshA, GGMATRIX* matA, sMesh* pMeshB, GGMATRIX* matB )
{
...
//			ConvertLocalMeshToTriList ( pMesh ); // moved above
		...
		for ( int iFace = 0; iFace < (int)pNewMesh->m_dwFaceCount; iFace++ )
		{
			...
			for ( int iVertex = 0; iVertex < 3; iVertex++ )
			{
				...
				//if ( matA )
				//	D3DXVec3TransformCoord ( ( GGVECTOR3* ) &pVertex, ( GGVECTOR3* ) &pVertex, matA );
			...
			GGVECTOR3 vec0 = GGVECTOR3 ( pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 0 ].x, pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 0 ].y, pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 0 ].z );
			GGVECTOR3 vec1 = GGVECTOR3 ( pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 1 ].x, pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 1 ].y, pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 1 ].z );
			GGVECTOR3 vec2 = GGVECTOR3 ( pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 2 ].x, pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 2 ].y, pNewMesh->m_ppFaces [ iFace ]->m_pVertices [ 2 ].z );

			// get the edges
			GGVECTOR3 edge1 = ( vec1 ) - ( vec0 );
			GGVECTOR3 edge2 = ( vec2 ) - ( vec0 );
			GGVECTOR3 vecNormal;
	...
	if ( pMeshB )
	{
		...
		for ( int iFace = 0; iFace < (int)pNewMesh->m_dwFaceCount; iFace++ )
		{
			...
			for ( int iVertex = 0; iVertex < 3; iVertex++ )
			{
				...
				// mike - 13083 - only transform if matrix is valid
				//if ( matB )
				//	D3DXVec3TransformCoord ( ( GGVECTOR3* ) &pVertex, ( GGVECTOR3* ) &pVertex, matB );
