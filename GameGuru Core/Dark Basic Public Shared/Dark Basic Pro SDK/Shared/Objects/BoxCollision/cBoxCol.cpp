...
GGVECTOR3 gvLatestStaticCollisionResult;
GGVECTOR3 gvLatestObjectCollisionResult;
GGVECTOR3 gvLightOfSight;
...
namespace
{
	...
	GGMATRIX CalcObjectRotationMatrix(sObject* Obj)
	{
		...
		else
		{
			...
			GGMatrixRotationX ( &matRotateX, GGToRadian ( Obj->position.vecRotate.x ) );	// x rotation
			GGMatrixRotationY ( &matRotateY, GGToRadian ( Obj->position.vecRotate.y ) );	// y rotation
			GGMatrixRotationZ ( &matRotateZ, GGToRadian ( Obj->position.vecRotate.z ) );	// z rotation
...
int CheckWorldBoxCollision ( GGVECTOR3 box1p, GGMATRIX matARotation, sCollisionData* pCollisionA, GGVECTOR3 box2p, GGMATRIX matBRotation, sCollisionData* pCollisionB )
{
	// Box 1
	GGVECTOR3 box1s;
	...
	GGVec3TransformCoord ( &box1offset, &box1offset,  &matARotation );
	box1p+=box1offset;
	...
	GGVec3TransformCoord ( &box2offset, &box2offset,  &matBRotation );
	box2p+=box2offset;
...
int CheckCollisionPool(float ax1, float ay1, float az1, float ax2, float ay2, float az2, float x1, float y1, float z1, float x2, float y2, float z2, GGVECTOR3* vResult)
{
	int iResult=0;
...
int CheckCollisionPoolWithLine(float sx, float sy, float sz, float dx, float dy, float dz, float width, float acc, GGVECTOR3* vResult)
{
	int iResult=0;
...
int CheckAdjustedAagainstB ( GGVECTOR3 box1p, sObject* pA, sObject* pB )
{
	// Work vars
	GGMATRIX matRotateX, matRotateY, matRotateZ;
...
//	box1offset.z=pA->collision.vecMin.z+box1s.z;
	...
	GGMATRIX matARotation = CalcObjectRotationMatrix( pA );
/*
	...
	{
		GGMatrixRotationX ( &matRotateX, GGToRadian ( pA->position.vecRotate.x ) );	// x rotation
		GGMatrixRotationY ( &matRotateY, GGToRadian ( pA->position.vecRotate.y ) );	// y rotation
		GGMatrixRotationZ ( &matRotateZ, GGToRadian ( pA->position.vecRotate.z ) );	// z rotation
...
//	GGVECTOR3 box2p = pB->position.vecPosition;
	GGVECTOR3 box2p = GGVECTOR3(pB->position.matWorld._41,pB->position.matWorld._42,pB->position.matWorld._43);
	GGVECTOR3 box2s;
...
//	box2offset.z=pB->collision.vecMin.z+box2s.z;
	...
	// 20120404 IanM - Use common method of calculating the object rotation matrix
	GGMATRIX matBRotation = CalcObjectRotationMatrix( pB );
/*
	GGMATRIX matBRotation;
	...
	{
		GGMatrixRotationX ( &matRotateX, GGToRadian ( pB->position.vecRotate.x ) );	// x rotation
		GGMatrixRotationY ( &matRotateY, GGToRadian ( pB->position.vecRotate.y ) );	// y rotation
		GGMatrixRotationZ ( &matRotateZ, GGToRadian ( pB->position.vecRotate.z ) );	// z rotation
...
int CheckBoxPairSystemCollision( sObject* pA, sObject* pB )
{
	...
	if(bYesRotated==true)
	{
		...
		GGVECTOR3 box1p = GGVECTOR3(pA->position.matWorld._41,pA->position.matWorld._42,pA->position.matWorld._43);
...
*/
			...
			float fDeterminant;
			GGMATRIX matInvBoxBRot;
			GGMatrixInverse ( &matInvBoxBRot, &fDeterminant, &matBRotation );

			// rotate Start and End vectors of A using B
			GGVECTOR3 vecAS = pA->position.vecLastPosition;
			GGVECTOR3 vecAE = pA->position.vecPosition;
			GGVECTOR3 vecOldDirection = vecAE - vecAS;
			GGVec3TransformCoord ( &vecAS, &vecAS, &matInvBoxBRot );
			GGVec3TransformCoord ( &vecAE, &vecAE, &matInvBoxBRot );

			// work out closest point in A
			GGVECTOR3 vecCLOSEST;
			float fDistClosest=9999.0f;
			GGVECTOR3 vecB = pB->position.vecPosition;
			for ( int iPoint=0; iPoint<4; iPoint++ )
			{
				// center of each side
				GGVECTOR3 vP = GGVECTOR3(0,0,0);
				...
				GGVec3TransformCoord ( &vP, &vP, &matARotation );
				vP+=box1p;
			...
			vecCLOSEST -= box2p;
			GGVec3TransformCoord ( &vecCLOSEST, &vecCLOSEST, &matInvBoxBRot );
			...
			GGVECTOR3 vecAdjustedDirection = vecAE - vecAS;

			// orient direction to box Bs rotation for new sliding data
			GGVec3TransformCoord ( &vecAdjustedDirection, &vecAdjustedDirection, &matBRotation );
