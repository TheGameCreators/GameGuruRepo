...
void ExtractFrustum(Frustum& frustum, const GGMATRIX& viewProjMatrix)
{
...
HRESULT LoadEffectFromFile(LPGGDEVICE pDevice, LPCWSTR pSrcFile, LPGGEFFECT* ppEffect)
{
...
void HZBManager::Render(const GGMATRIX& view, const GGMATRIX& projection)
{
    RenderOccluders(view, projection);
    RenderMipmap();
}

void HZBManager::RenderOccluders(const GGMATRIX& view, const GGMATRIX& projection)
{
	// camera zero to work out distance for occluder chooser
	tagCameraData* m_Camera_Ptr = (tagCameraData*)GetCameraInternalData ( 0 );
	GGVECTOR3* pvecCamPos = &m_Camera_Ptr->vecPosition;
    ...
    for ( DWORD iObjOccluderList = 0; iObjOccluderList < m_vObjectOccluderList.size(); ++iObjOccluderList )
    {
		...
		if ( pOccObject )
		{
			...
			GGVECTOR3* pObjPos = &pOccObject->pObject->position.vecPosition;
    ...
    hr = m_pDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, GGCOLOR_ARGB( 255, 255, 255, 255 ), 1.0f, 0 );
    assert(SUCCEEDED(hr));
	...
	GGMATRIX occluderWorld;
	D3DXMatrixIdentity(&occluderWorld);
	GGMATRIX WVP = occluderWorld * view * projection;

	hr = m_pDepthEffect->SetMatrix(m_hDepthEffectParamWVP, &WVP);
	assert(SUCCEEDED(hr));

	GGMATRIX WV = occluderWorld * view;
	...
	if ( 1 )
	{
		LPGGTEXTURE pDebugSee = GetImagePointer ( 59961 );
		if ( pDebugSee )
		{
			for ( int mips=0; mips<3; mips++ )
			{
				LPGGSURFACE pOccluderDebugImage;
...
void HZBManager::RenderMipmap()
{
    ...
    for (UINT miplevel = 1; miplevel < m_mipLevels; miplevel++)
    {
        ...
        GGVECTOR4 lastMipInfo(lastWidth, lastHeight, miplevel - 1, 0);
    ...
    const GGMATRIX& projection)
{
    ...
    {
        v[i].Bound = bounds[i];
        v[i].Pixel = GGVECTOR2(i % m_cullingResultsWidth, i / m_cullingResultsWidth);
    }
	...
	GGMATRIX VP = view * projection;
    hr = m_pHizCullEffect->SetMatrix(m_hCullingParamViewProjection, &VP);
    assert(SUCCEEDED(hr));

    Frustum frustum;
    ExtractFrustum(frustum, VP);
    hr = m_pHizCullEffect->SetVectorArray(m_hCullingParamFrustumPlanes, (GGVECTOR4*)&frustum, 6);
    assert(SUCCEEDED(hr));

    GGVECTOR4 viewport(m_width, m_height, 0, 0);
    hr = m_pHizCullEffect->SetVector(m_hCullingParamViewportSize, &viewport);
    assert(SUCCEEDED(hr));

    GGVECTOR4 resultsSize(m_cullingResultsWidth, m_cullingResultsHeight, 0, 0);
...
void HZBManager::LoadDepthEffect()
{
    ...
    GGVERTEXELEMENT pElements[] = {
        {0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0},
        D3DDECL_END()
    };

    hr = m_pDevice->CreateVertexDeclaration((GGVERTEXELEMENT*)&pElements, &m_pDepthVertexDecl);
    assert(SUCCEEDED(hr));
	...
	GGFORMAT DepthFormat = m_ObjectManager.GetValidStencilBufferFormat(CommonFormat);
				...
				if ( pMesh )
				{
					...
					else
					{
						...
						if ( dwIndexCount>0 )
						{
							...
							{
								GGVECTOR3 vec = *(GGVECTOR3*)(pSrcPtrAt+(pIndexLookup[i]*dwSizeInFloats));
						...
						else
						{
							...
							{
								GGVECTOR3 vec = *(GGVECTOR3*)pSrcPtrAt;
								D3DXVec3TransformCoord ( &vec, &vec, &matFinal );
								vec.x += pObject->position.vecPosition.x;
								vec.y += pObject->position.vecPosition.y;
								vec.z += pObject->position.vecPosition.z;
...
void HZBManager::LoadMipmapEffect()
{
    ...
    hr = m_pDevice->CreateVertexDeclaration((GGVERTEXELEMENT*)&pElements, &m_pHizMipmapVertexDecl);
    assert(SUCCEEDED(hr));
...
void HZBManager::LoadCullEffect()
{
    ...
    hr = m_pDevice->CreateVertexDeclaration((GGVERTEXELEMENT*)&pElements, &m_pCullingBoundsVertexDecl);
    assert(SUCCEEDED(hr));
}