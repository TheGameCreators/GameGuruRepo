...
DARKSDK_DLL void CloneObject ( int iDestinationID, int iSourceID, int iCloneSharedData )
{
	...
	if ( iCloneSharedData!=101 )
	{
		...
		/* the destination bounds already calculated inside FinalProperties, and the source may
		have been corrupted by moving it to a corner of the world, changing matCombined and messing up bound calc (parent objects)
		if ( pObject->pAnimationSet )
		{
			if ( pObject->pAnimationSet->pvecBoundMin )
			{
				...
				pNewObject->pAnimationSet->pvecBoundMin = new GGVECTOR3 [ dwTotalFrames ];
				SAFE_DELETE(pNewObject->pAnimationSet->pvecBoundMax);
				pNewObject->pAnimationSet->pvecBoundMax = new GGVECTOR3 [ dwTotalFrames ];
				SAFE_DELETE(pNewObject->pAnimationSet->pvecBoundCenter);
				pNewObject->pAnimationSet->pvecBoundCenter = new GGVECTOR3 [ dwTotalFrames ];
	...
	if ( iCloneSharedData!=101 )
	{
		...
		//Dave, copying over properties that are missing
		//Dave - commented back out as we don't seem to need these set - and time is precious
		/*g_ObjectList [ iDestinationID ]->bInstanceAlphaOverride = pObject->bInstanceAlphaOverride;
		g_ObjectList [ iDestinationID ]->bTransparentObject = pObject->bTransparentObject;
		g_ObjectList [ iDestinationID ]->bOverlayObject = pObject->bOverlayObject;
		g_ObjectList [ iDestinationID ]->bTransparencyWaterLine = pObject->bTransparencyWaterLine;		*/
	}
...
DARKSDK_DLL void MakeObjectPlane ( int iID, float fWidth, float fHeight, int iFlag )
{
    if (iFlag == 0)
    {
        MakeObjectPlane ( iID, fWidth, fHeight );
        return;
    }
...
DARKSDK_DLL void AppendObject ( SDK_LPSTR pString, int iID, int iFrame )
{
	// check the object exists
	if ( !ConfirmObject ( iID ) )
		return;
...
DARKSDK_DLL void SaveObjectAnimation ( int iID, SDK_LPSTR pFilename )
{
	// Not Implemented in DBPRO V1 RELEASE
	RunTimeError(RUNTIMEERROR_COMMANDNOWOBSOLETE);
}

DARKSDK_DLL void AppendObjectAnimation ( int iID, SDK_LPSTR pFilename )
{
	// Not Implemented in DBPRO V1 RELEASE
	RunTimeError(RUNTIMEERROR_COMMANDNOWOBSOLETE);
}
...
{
	SetEffectToShadowMappingEx ( iEffectID, 0, 0, 1, 1024, 4, 2, 8, 16, 75, 100, 100, 100, 100 );
}
...
{
	GGVECTOR3 vecBob = pObject->position.vecPosition;
	...
	GGVec3TransformCoord(&vecBob, &vecBob, &matTransform);
