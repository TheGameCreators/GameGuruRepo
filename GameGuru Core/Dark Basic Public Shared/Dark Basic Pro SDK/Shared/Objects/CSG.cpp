...
void TransformVertices ( sMesh* pMesh, GGMATRIX matWorld );
...
bool cCSG::Union ( sMesh* pMeshA, GGMATRIX OriginalMatA, sMesh* pMeshB, GGMATRIX OriginalMatB )
{
...
bool cCSG::Difference ( sMesh* pMeshA, GGMATRIX OriginalMatA, sMesh* pMeshB, GGMATRIX OriginalMatB )
{
	...
	GenerateNormals ( pMeshA, 0 );
	GenerateNormals ( pMeshB, 0 );
	
	// all finished
	return true;
}

bool cCSG::Intersection ( sMesh* pMeshA, GGMATRIX OriginalMatA, sMesh* pMeshB, GGMATRIX OriginalMatB )
{
	// check parameters
	if ( !pMeshA || !pMeshB )
		return false;
...
void ClipMeshWithBoundArea ( sMesh* pMesh, GGVECTOR3* pvecMin, GGVECTOR3* pvecMax )
{
	...
	for ( DWORD i = 0; i < dwNumFaces; i++ )
	{
		...
		GGVECTOR3 v[3];
		v[0] = *(GGVECTOR3*)((float*)pBase+(wFace0*offsetMap.dwSize));
		v[1] = *(GGVECTOR3*)((float*)pBase+(wFace1*offsetMap.dwSize));
		v[2] = *(GGVECTOR3*)((float*)pBase+(wFace2*offsetMap.dwSize));
...
bool cCSG::Clip ( sMesh* pMeshA, GGMATRIX OriginalMatA, sMesh* pMeshB, GGMATRIX OriginalMatB )
{
	// check parameters
	if ( !pMeshA || !pMeshB )
		return false;
	...
	// get the new mesh bounds
	GGVECTOR3 vecMinA = pMeshA->Collision.vecMin;
	GGVECTOR3 vecMaxA = pMeshA->Collision.vecMax;
	D3DXVec3TransformCoord ( &vecMinA, &vecMinA, &OriginalMatA );
	D3DXVec3TransformCoord ( &vecMaxA, &vecMaxA, &OriginalMatA );
	GGVECTOR3 vecMinB = pMeshB->Collision.vecMin;
	GGVECTOR3 vecMaxB = pMeshB->Collision.vecMax;
...
void TransformVertices ( sMesh* pMesh, GGMATRIX matWorld )
{
	...
	for ( int iVertex = 0; iVertex < ( int ) pMesh->dwVertexCount; iVertex++ )
	{
		...
		if ( offsetMap.dwZ!=0 )
		{
			GGVECTOR3 vecPosition = GGVECTOR3 ( 
