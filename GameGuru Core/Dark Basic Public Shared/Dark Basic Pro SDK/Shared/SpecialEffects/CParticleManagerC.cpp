...
extern GGMATERIAL				gWhiteMaterial;
		...
		if ( ptr->m_bVisible==true )
		{
			...
			else
			{
				...
				while ( ptr->m_dwParticles < ptr->m_dwParticlesLim && ptr->m_dwParticles < dwParticlesEmit )
				{
					...
					pParticle->m_vPos0 = ptr->vPosition;// + GGVECTOR3( 0.0f, ptr->m_fRadius, 0.0f );
...
// leechange - 030306 - u60 - now take distance into account
					...
					while ( pParticle )
					{
						GGVECTOR3 vPos(pParticle->m_vPos);
						GGVECTOR3 vVel(pParticle->m_vVel);
