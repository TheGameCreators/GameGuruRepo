...
extern "C" void GG_GetRealPath( char* fullPath, int create );
...
char processing_filename[MAX_PATH];
...
int direct_load_time = 0; //PE: For debug.
int mipgen_load_time = 0;
int png_load_time = 0;
int unicode_load_time = 0;
int png_files_loaded = 0, mipgen_files_loaded = 0, direct_files_loaded = 0, unicode_files_loaded = 0;
...
DARKSDK LPGGTEXTURE GetTextureCore ( char* szFilename, GGIMAGE_INFO* info, int iOneToOnePixels, int iFullTexturePlateMode, int iDivideTextureSize )
{
	...
	else
	{
		...
		{
			bool bIsDDS = false;
			if (strnicmp(szFilename + strlen(szFilename) - 4, ".dds", 4) == NULL) bIsDDS = true;
			...
			//if (pPreCreatedTexture == NULL) { // && bIsDDS) {
			//	char mdebug[2048];
			//	sprintf(mdebug, "DLOAD POSSIBLE:(%d) %s ", m_iMipMapNum, szFilename);
			//	timestampactivity(0, mdebug);
			//}

			//if (strstr(processing_filename, "mud_") > 0) //PE: debug.

			if (pPreCreatedTexture != NULL && m_iMipMapNum == -1 && bIsDDS) {
				//PE: Already loaded with mipmaps generated.
				lpTexture = pPreCreatedTexture;
				pPreCreatedTexture = NULL;
			}
			else if (bIsDDS && bFormatDDSLoadFriendly == true )
			{
				...
				if ( iDivideTextureSize <= 1 )
				{
					...
					else
					{
						...
						else
						{
							...
							{
								if ( (unsigned char) szRealFilename[i] > 127)
								{
									bUnicode = true;
									break;
								}
							...
							if (bUnicode)
							{
								...
								//PE: some files fails ?
								//PE: PATH not found ...s\für hub level\schild level 1\junksign3_d.dds
								//PE: Some unicode chars like 'für' cant be found by CreateDDSTextureFromFile ?. D3DX11CreateTextureFromFile works :)
								D3DX11_IMAGE_LOAD_INFO loadinfo;
								...
								int ttime = Timer() - t;
								unicode_load_time += ttime;
								if (lpTexture != NULL)
								{
									unicode_files_loaded++;
									//char mdebug[2048];
									//sprintf(mdebug, "LOADTIME3 (%d) FILE: %s ", ttime, processing_filename);
									//timestampactivity(0, mdebug);
								}
							}
						}
			...
			else
			{
				...
				if ( m_iMipMapNum == -1 && (*info).MipLevels == 1 && strnicmp(szFilename + strlen(szFilename) - 4, ".dds", 4) == NULL ) 
				{
					...
					else
					{
						...
						hRes = D3DX11CreateTextureFromFile(m_pD3D, szRealFilename, &loadinfo, NULL, &lpTexture, NULL);
						int ttime = Timer() - t;
						mipgen_load_time += ttime;
						if (lpTexture != NULL)
						{
							mipgen_files_loaded++;
						}
						//char mdebug[2048];
						//sprintf(mdebug, "LOADTIME1 (%d) FILE: %s ", ttime, processing_filename);
						//timestampactivity(0, mdebug);
				...
				else 
				{
					...
					else
					{
						...
						int ttime = Timer() - t;
						png_load_time += ttime;
						if (lpTexture != NULL && m_iMipMapNum == -1)
						{
							png_files_loaded++;
						}
						//char mdebug[2048];
						//sprintf(mdebug, "LOADTIME2 (%d) FILE: %s ", ttime, processing_filename);
						//timestampactivity(0, mdebug);
					}
				}
			}
...
DARKSDK int LoadImageCoreInternal ( char* szFilename, int iDivideTextureSize )
{
	...
	if ( !FindInternalImage ( szFilename, &iImageID ) )
	{
		...
		strcpy(processing_filename, szFilename);
...
int GetImageFileExist ( LPSTR pFilename )
{
	...
	//PE: GG_CreateFile to slow on classic.
...
void ImageCreateSurfaceTextureChannels(LPSTR pSurfaceToSave, LPSTR pAO, LPSTR pGloss, LPSTR pMetalness, int iO, int iG, int iM, int iA )
{
	...
	for (int contributiontex = 0; contributiontex < 3; contributiontex++ )
	{
		...
		if (pTheFilename && *pTheFilename)
		{
			...
			if (info.format >= DXGI_FORMAT_BC1_TYPELESS && info.format <= DXGI_FORMAT_BC5_SNORM)
			{
				...
				hr = DirectX::Decompress(image->GetImages(), image->GetImageCount(), image->GetMetadata(), DXGI_FORMAT_R8G8B8A8_UNORM, *scratchImages[contributiontex] );
				image->Release();
				delete image;
			}
			else
			{
				if (info.format != DXGI_FORMAT_R8G8B8A8_UNORM)
				{
					// must be in RGBA format, so convert
					scratchImages[ contributiontex ] = new DirectX::ScratchImage();
					hr = DirectX::Convert(image->GetImages(), image->GetImageCount(), image->GetMetadata(), DXGI_FORMAT_R8G8B8A8_UNORM, DirectX::TEX_FILTER_DITHER, DirectX::TEX_THRESHOLD_DEFAULT, *scratchImages[contributiontex]);
					image->Release();
					delete image;
				}
				else
	...
	DirectX::ScratchImage result;
	hr = result.Initialize2D(DXGI_FORMAT_R8G8B8A8_UNORM, iResultSurfaceWidth, iResultSurfaceHeight, 1, 1, DirectX::DDS_FLAGS_NONE);
	...
	if ( hr == S_OK )
	{
		...
		if ( SUCCEEDED(hr) )
		{
			...
			if ( hr != S_OK )
			{
				...
				imageTexturePlate.Initialize2D ( DXGI_FORMAT_R8G8B8A8_UNORM, finfo.Width, finfo.Height, 1, 1, 0 );
...
LPGGTEXTURE ConvertBackBufferToNewFormat ( LPGGSURFACE pBackBuffer, GGFORMAT NewFormat)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		//hr = DirectX::Convert(imageBBSurface.GetImages(), imageBBSurface.GetImageCount(), imageBBSurface.GetMetadata(), DXGI_FORMAT_R8G8B8A8_UNORM, DirectX::TEX_FILTER_DITHER, DirectX::TEX_THRESHOLD_DEFAULT, imageBBSurfaceConverted);
...
DARKSDK unsigned char* DecompressImage ( LPGGSURFACE pTexSurface )
{
	...
	if ( pTexSurface )
	{
		...
		if ( SUCCEEDED ( hr ) )
		{
			...
			hr = Decompress ( imageTexturePlate.GetImages ( ), imageTexturePlate.GetImageCount ( ), imageTexturePlate.GetMetadata ( ),
				DXGI_FORMAT_R8G8B8A8_UNORM, uncompressedTexturePlate );
			if (FAILED(hr))
			{
