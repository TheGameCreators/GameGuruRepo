...
///extern DBPRO_GLOBAL PTR_FuncCreateStr		g_pCreateDeleteStringFunction;
...
DARKSDK int DMEMAvailable(void)
{
					...
					for (i = 0; i < segmentCount; i++) 
					{
						...
						if (queryD3DKMTStatistics(&queryStatistics)==0) 
						{
							...
							if ( IsWindows8OrGreater() )
							{
								...
								if ( queryStatistics.QueryResult.SegmentInformation.Aperture == 0 )
									aperture = false;
								else
									aperture = true;
							}
							...
							{
								// Windows 7
								if ( queryStatistics.QueryResult.SegmentInformationV1.Aperture == 0 )
									aperture = false;
								else
									aperture = true;
							}
	...
	Memory = (int)(dedicatedBytesUsed / 1024 / 1024);
	return Memory;
	#endif
	return 0;
}
...
DARKSDK bool CheckListForValue ( int iIndex )
{
	...
	{
		//RunTimeError(RUNTIMEERROR_CHECKLISTNUMBERWRONG); // 080917 - make it a silent fail in case try to get collision checklist item after IntersectAll
		return false;
	}
	return true;
}
...
{
	if ( !CheckListForValue ( iIndex ) ) return 0;
	return g_pGlob->checklist[iIndex-1].fvaluea;
}

DARKSDK float ChecklistFValueB ( int iIndex )
{
	if ( !CheckListForValue ( iIndex ) ) return 0;
	return g_pGlob->checklist[iIndex-1].fvalueb;
}

DARKSDK float ChecklistFValueC ( int iIndex )
{
	if ( !CheckListForValue ( iIndex ) ) return 0;
	return g_pGlob->checklist[iIndex-1].fvaluec;
}

DARKSDK float ChecklistFValueD ( int iIndex )
{
	if ( !CheckListForValue ( iIndex ) ) return 0;
	return g_pGlob->checklist[iIndex-1].fvalued;
}
...
DARKSDK LPSTR ChecklistString( int iIndex )
{
	...
	{
		pReturnString=GetReturnStringFromTEXTWorkString(g_pGlob->checklist[iIndex-1].string);
	}
...
DARKSDK void DLLLoadCore( LPSTR pDLLFile, int dllid )
{
	// Load a DLL (LOAD DLL)
	if(dllid>=1 && dllid<=255)
	{
		...
		{
			hDLLModule[dllid] = LoadLibrary(pDLLFile);
			if(!hDLLModule[dllid])
			{
