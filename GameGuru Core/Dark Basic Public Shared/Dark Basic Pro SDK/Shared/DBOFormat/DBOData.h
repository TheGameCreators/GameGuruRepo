...
#define GG_TEXTURE_CHANNEL_MASK_R	0x08
#define GG_TEXTURE_CHANNEL_MASK_G	0x04
#define GG_TEXTURE_CHANNEL_MASK_B	0x02
#define GG_TEXTURE_CHANNEL_MASK_A	0x01
#define GG_TEXTURE_CHANNEL_INDEX(mask)	(((mask) >> 4) & 0x3)
...
struct sMultiMaterial
{
	...
	LPGGTEXTUREREF					pTexturesRefM;								// 220220 - added to support MASK and ALTALBEDO functionality
...
// index into sMeshTexture::pTextures array
#define GG_MESH_TEXTURE_DIFFUSE     0
#define GG_MESH_TEXTURE_NORMAL      1
#define GG_MESH_TEXTURE_METALNESS   2
#define GG_MESH_TEXTURE_ROUGHNESS   3
#define GG_MESH_TEXTURE_OCCLUSION   4
#define GG_MESH_TEXTURE_EMISSIVE    5
#define GG_MESH_TEXTURE_SURFACE	    6 // computed from Occlusion + Roughness + Metalness, stored for performance
...
struct sMeshExternalProperties
{
	...
	bool							bProtectZWriteState;						// 010220 - added so noZwrite state is never overwritten (used to be bGhost)
	bool							bVisible;									// is object visible
	bool							bZRead;										// z buffer
	bool							bZWrite;									// z write
	int								iReservedForFuture;							// 010220 - reserved (used to be iGhostMode)
					  ...
					  sMeshMultipleAnimation
{
	...
	float							fLastAlphaOverride;							// 220720 - so can skip applying same alpha over and over
					   ...
					   sFramePosition
{
	...
	// reserved members
	GGMATRIX*						pmatBoneLocalTransform;
	sAnimation*						pAnimRef;
...
struct sAnimationSet
{
	...
	// reserved members
	DWORD							dwAnimSetType;								// LB: added to determine type of animset DBO is storing
	float							fAnimSetStart;								// LB: added start float
	float							fAnimSetFinish;								// LB: added finish float
						...
						sObjectCustom
{
	...
	bool bLightDateUpdated = false;
	int iDelayedUpdate = 0;
};
