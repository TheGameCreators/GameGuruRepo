...
DARKSDK_DLL bool WriteVECTOR ( GGVECTOR3 vecVector, LPSTR* ppBlock, DWORD* pdwSize )
{
...
DARKSDK_DLL bool WriteMATRIX ( GGMATRIX mMatrix, LPSTR* ppBlock, DWORD* pdwSize )
{
...
DARKSDK_DLL bool ReadBYTE ( unsigned char* ppDest, LPSTR* ppBlock )
{
	*(ppDest) = *(unsigned char*)*ppBlock;
	*ppBlock += 1;
	return true;
}
...
DARKSDK_DLL bool ReadVector ( GGVECTOR3* pVector, LPSTR* ppBlock )
{
...
DARKSDK_DLL bool ReadMatrix ( GGMATRIX* pMatrix, LPSTR* ppBlock )
{
...
//		WriteCODE	( DBOBLOCK_MESH_DRAWPRIMCOUNT, pMesh->dwIndexCount*sizeof(WORD), ppBlock, pdwSize );
		...
		// 010217 - new property to DBO to hold specular override value
		DWORD dwDWORD = *(DWORD*)&pMesh->fSpecularOverride;
		WriteCODE	( DBOBLOCK_MESH_SPECULAROVERRIDE, 4,	ppBlock, pdwSize );
		...
		WriteBYTE	( pMesh->bProtectZWriteState,	ppBlock, pdwSize );
		WriteCR	( ppBlock, pdwSize );
		WriteCODE	( DBOBLOCK_MESH_GHOSTMODE,	4,	ppBlock, pdwSize );
		WriteDWORD	( pMesh->iReservedForFuture,	ppBlock, pdwSize );
		WriteCR	( ppBlock, pdwSize );
		...
		WriteCR	( ppBlock, pdwSize );
		dwDWORD = *(DWORD*)&pMesh->fZBiasSlopeScale;
...
DARKSDK_DLL bool ReadBoneMatrices ( GGMATRIX** ppMatrix, DWORD dwA, DWORD dwB, LPSTR* ppBlock )
{
	DWORD dwLength = sizeof ( GGMATRIX );

	for ( int iFrame = 0; iFrame < (int)dwA; iFrame++ )
	{
		ppMatrix [ iFrame ] = new GGMATRIX [ dwB ];
	}
...
DARKSDK_DLL bool ScanAnimationSet ( sAnimationSet* pAnimSet, LPSTR* ppBlock, DWORD* pdwSize )
{
	...
	if ( pAnimSet )
	{
		...
		// Write type
		WriteCODE(DBOBLOCK_ANIMSET_TYPE, 4, ppBlock, pdwSize);
		WriteDWORD((DWORD)pAnimSet->dwAnimSetType, ppBlock, pdwSize);
		WriteCR(ppBlock, pdwSize);

		// Write anim start
		WriteCODE(DBOBLOCK_ANIMSET_START, 4, ppBlock, pdwSize);
		WriteDWORD(*(DWORD*)&pAnimSet->fAnimSetStart, ppBlock, pdwSize);
		WriteCR(ppBlock, pdwSize);
		
		// Write anim finish
		WriteCODE(DBOBLOCK_ANIMSET_FINISH, 4, ppBlock, pdwSize);
		WriteDWORD(*(DWORD*)&pAnimSet->fAnimSetFinish, ppBlock, pdwSize);
		WriteCR(ppBlock, pdwSize);
		...
		// Write NEXT animset (if present)
		if (pAnimSet->pNext)
		{
			pCodeSizePtr = NULL;
			if (ppBlock) pCodeSizePtr = *ppBlock + 4;
			WriteCODE(DBOBLOCK_ANIMSET_NEXT, 0, ppBlock, pdwSize);
			WriteCR(ppBlock, pdwSize);
			if (pAnimSet->pNext) ScanAnimationSet(pAnimSet->pNext, ppBlock, pdwSize);
			WriteCR(ppBlock, pdwSize);
			if (pCodeSizePtr) *(DWORD*)pCodeSizePtr = (*ppBlock - 4) - pCodeSizePtr;
		}
	}
...
DARKSDK_DLL bool ConstructMesh ( sMesh** ppMesh, LPSTR* ppBlock )
{
	...
	while ( dwCode > 0 )
	{
		switch ( dwCode )
		{
			...
			case DBOBLOCK_MESH_SPECULAROVERRIDE : ReadDWORD            ( (DWORD*)&(*ppMesh)->fSpecularOverride, ppBlock );							break;
			...
			case DBOBLOCK_MESH_GHOST :			ReadBOOL               ( &(*ppMesh)->bProtectZWriteState, ppBlock );												break;
			case DBOBLOCK_MESH_GHOSTMODE :		ReadDWORD              ( (DWORD*)&(*ppMesh)->iReservedForFuture, ppBlock );									break;
	...
	while ( dwCode > 0 )
	{
		switch ( dwCode )
		{
			...
			case DBOBLOCK_FRAME_BONEMATRIXLIST:
				(*ppAnim)->ppBoneFrames = new GGMATRIX* [ (*ppAnim)->iBoneFrameA];
...
DARKSDK_DLL bool ConstructAnimationSet ( sAnimationSet** ppAnimSet, LPSTR* ppBlock )
{
	...
	while ( dwCode > 0 )
	{
		switch ( dwCode )
		{
			...
			case DBOBLOCK_ANIMSET_TYPE:		ReadDWORD(&(*ppAnimSet)->dwAnimSetType, ppBlock);			break;
			case DBOBLOCK_ANIMSET_NEXT:		ConstructAnimationSet(&(*ppAnimSet)->pNext, ppBlock);		break;
			case DBOBLOCK_ANIMSET_START:	ReadDWORD((DWORD*)&(*ppAnimSet)->fAnimSetStart, ppBlock);	break;
			case DBOBLOCK_ANIMSET_FINISH:	ReadDWORD((DWORD*)&(*ppAnimSet)->fAnimSetFinish, ppBlock);	break;
