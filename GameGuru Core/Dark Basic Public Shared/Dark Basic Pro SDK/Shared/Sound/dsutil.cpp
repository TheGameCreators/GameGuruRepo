...
LPDIRECTSOUNDBUFFER* CSoundManager::LoadOggVorbis ( char* dwFileName, DWORD* pdwSizeOfSound )
{
	...
	{
		//Dave - close file before exiting
		fclose(fp);
		return NULL;
	}
	...
	*pdwSizeOfSound = (DWORD)iPCMTotalSize * wfm.nBlockAlign;
	...
	// close the file
	//Dave - commented this out as you must not use flclose fs used ov_open(), instead should use ov_clear()
	//fclose ( fp );
	ov_clear(&OggVorbis);
