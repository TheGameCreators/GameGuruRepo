...
bool CFileReader::MakePCKFromEXE(LPSTR ActualEXEFilename, LPSTR pPCKFilename)
{
	...
	else
	{
		...
		HANDLE hfile = GG_CreateFile(ActualEXEFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		if(ValidityValue==12345678)
		{
			...
			hfile = GG_CreateFile(pPCKFilename, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
			if(hfile!=INVALID_HANDLE_VALUE)
			{
...
{
	// Load Data segment into memory
	HANDLE hfile = GG_CreateFile(pPCKFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if(hfile==INVALID_HANDLE_VALUE)
		return false;
...
bool CFileReader::CreateFileFromData(LPSTR UseFilename, char* dataptr, DWORD datasize)
{
	...
	HANDLE hfile = GG_CreateFile(UseFilename, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if(hfile==INVALID_HANDLE_VALUE)
		return false;
