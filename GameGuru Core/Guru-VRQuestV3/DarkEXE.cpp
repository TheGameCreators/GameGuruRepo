...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	//Ignore all launch checks if this is a standalone or using any other type of commands.
	//Only allow one instance of app to run if we have no '-' on command line.

	bool bOnlyAllowOneCopyToRun = true;
	extern char launchLoadOnStartup[260];
	strcpy(launchLoadOnStartup, "");

	if (lpCmdLine && _tcslen(lpCmdLine) > 0) {
		//size_t size = wcstombs(NULL, lpCmdLine, 0);
		int size = strlen(lpCmdLine);
		char* CharStr = new char[size + 1];
		//wcstombs(CharStr, (const wchar_t *)lpCmdLine, size + 1);
		strcpy(CharStr, lpCmdLine);
		CharStr[size] = 0;
		if (CharStr[0] != '-' && CharStr[1] != '-') {
			//No commands , assume fpe.
			if (pestrcasestr(CharStr, ".fpm")) {
				//remove quotes if any.
				if (CharStr[0] == '\"')
					strcpy(launchLoadOnStartup, CharStr + 1);
				else
					strcpy(launchLoadOnStartup, CharStr);
				if (launchLoadOnStartup[strlen(launchLoadOnStartup) - 1] == '\"')
					launchLoadOnStartup[strlen(launchLoadOnStartup) - 1] = 0;

				//MessageBoxA(NULL, launchLoadOnStartup, "LAUNCH", 0);

				if (strlen(ActualEXEFilename) > 0) {
					//MessageBoxA(NULL, ActualEXEFilename, "PATH", 0);
					//We need to change current folder to where .exe is.
					char cExeDir[MAX_PATH];
					strcpy(cExeDir, ActualEXEFilename);
					for (int i = strlen(cExeDir); i > 0; i--) {
						if (cExeDir[i] == '\\' || cExeDir[i] == '/') {
							cExeDir[i] = 0;
							break;
						}
					}
			...
			bOnlyAllowOneCopyToRun = false;
		}
		delete CharStr;
	}

	try {
		if (bOnlyAllowOneCopyToRun) {
			// Try to open the mutex.
			hMutex = OpenMutex(MUTEX_ALL_ACCESS, 0, "GameGuruUniqueId20427509String");
			if (!hMutex)
				// Mutex doesn’t exist. This is the first instance so create.
				hMutex = CreateMutex(0, 0, "GameGuruUniqueId20427509String");
			else {
				// The mutex exists so this is the the second instance.
				HWND hWnd = FindWindow("GameGuru12345", NULL); // always this when in editor.
				if (hWnd != NULL) {
					SetForegroundWindow(hWnd);

					if (lpCmdLine && _tcslen(lpCmdLine) > 0) {
						COPYDATASTRUCT cds;
						//size_t size = wcstombs(NULL, (const wchar_t *) lpCmdLine, 0);
						int size = strlen(lpCmdLine);
						char* CharStr = new char[size + 1];
						//wcstombs(CharStr, (const wchar_t *) lpCmdLine, size + 1);
						strcpy(CharStr, lpCmdLine);
	...
	//Load setup.
	FILE* preffile = GG_fopen(prefile, "rb");
	if (preffile) {
	...
	if (bResetPreferences)
	{
		...
		pref.launched = 0;
		pref.current_style = 3; //Default to light style
