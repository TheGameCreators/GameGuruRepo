...
{
	HANDLE hFile = GG_CreateFile(lpFilename, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if(hFile!=INVALID_HANDLE_VALUE)
	{
		// Settings
...
{
	// Load EXE Filedata
	HANDLE hFile = GG_CreateFile(lpFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if(hFile!=INVALID_HANDLE_VALUE)
	{
		// Settings
...
bool CEXEBlock::Init ( HINSTANCE hInstance, bool bResult, LPSTR pCmdLine )
{
	...
	// load icon from root folder
	if(g_pGlob)
	{
		// Seems this command not recognised now!?! LoadIcon(g_pGlob->hInstance, MAKEINTRESOURCE(IDI_ICON1));
		char pIconFromRootFolder[MAX_PATH];
		sprintf(pIconFromRootFolder, "%s\\icon.ico", m_pOriginalFolderName);
		g_pGlob->hAppIcon = (HICON)LoadImageA(hInstance, pIconFromRootFolder, IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
	}
