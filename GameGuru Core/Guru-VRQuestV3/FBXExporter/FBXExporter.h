...
private:
	...
	FbxLongLong mAnimationLengthThisTime;
	std::string mAnimationName;
	LARGE_INTEGER mCPUFreq;
	int mMeshCount;

private:
	void ProcessGeometry(FbxNode* inNode,sObject* pObject);
	void ProcessMeshHierarchy(FbxNode* inRootNode);
	void ProcessMeshHierarchyRecursively(FbxNode* inNode);
	void ProcessSkeletonHierarchy(FbxNode* inRootNode);
	void ProcessSkeletonHierarchyRecursively(FbxNode* inNode, int inDepth, int myIndex, int inParentIndex);
	void ProcessControlPoints(FbxNode* inNode);
	int ProcessJointsAndAnimations(FbxNode* inNode);
