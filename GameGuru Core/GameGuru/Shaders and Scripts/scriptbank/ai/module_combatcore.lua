...
function module_combatcore.detectplayer(e,AIObjNo,PlayerDist,CanFire,detectstate,combattype)
    ...
    if module_combatcore.getcomfortdistance(e) < 25 then
     ...
     if AIGetEntityIsMoving(AIObjNo) == 1 then
      ...
      if g_Entity[e]['plrvisible'] == 1 then
	   if ai_bot_state[e] == ai_state_idle and combattype ~= ai_combattype_bashmelee then
	    -- if not moving and can see, fire on spot!
...
function module_combatcore.idle(e,AIObjNo,PlayerDist,CanFire,detectstate,combattype)
 ...
 --if g_Entity[e]['plrvisible'] == 0 and PlayerDist < 200 then Another Duff Logic - dont stop a chase, even if enemy cannot see the player!
 -- ai_bot_state[e] = ai_state_idle
 -- SetAnimation(0)
 -- CanFire = 0
 -- module_combatcore.donotmove(e) 
...
function module_combatcore.patrol(e,AIObjNo,PlayerDist,MoveType,CanFire,detectstate,stopstate,combattype)
 ...
 if ai_bot_state[e] == ai_state_patrol then
  ...
  if DistFromPath < 50 and VertDist < 95 and g_Time > ai_bot_pointtime[e] then
   ai_bot_pointtime[e] = g_Time + 100
   AISetEntityMoveBoostPriority(AIObjNo)
   StartTimer(e)
...
function module_combatcore.hunt(e,AIObjNo,PlayerDist,MoveType,CanFire,stopstate,duckstrafe)
 ...
 if g_Entity[e]['plrvisible'] == 1 then 
  ...
  if ai_bot_hunttime[e] ~= nil then
   if ai_bot_hunttime[e] > 0 and Timer() > ai_bot_hunttime[e]+10000 then
    if ai_bot_state[e] ~= ai_state_duck and ai_bot_state[e] ~= ai_state_patrol then
     ai_bot_state[e] = ai_state_startmove
     ai_bot_hunttime[e] = 0
     SetActivated(e,0)
    end
  ...
  --if g_Entity[e]['plrvisible'] == 0 and PlayerDist < 200 then Duff Logic - enemy could be chasing player around a maze and getting VERY close without seeing them, do not interfere!
  -- ai_bot_state[e] = ai_state_idle
  -- SetAnimation(0)
  -- CanFire = 0
  -- module_combatcore.donotmove(e) 
 ...
 if ai_bot_state[e] == ai_state_duckstart then
  ...
  if ai_bot_state[e] == ai_state_duck then
   ...
   if ai_bot_substate[e] == 1 then
	...
	-- if ducked for a while, and nothing happening, unduck
	if GetTimer(e) > 3000 and g_Entity[e]['plrvisible'] == 0 then
	 ai_bot_state[e] = ai_state_unduckstart
	end
   end
...
function module_combatcore.moveandavoid(e,AIObjNo,PlayerDist,MoveType,x,y,z,stopstate)
 ...
 if ai_bot_substate[e] == 0 then
  ...
  SetRotationYSlowly(e,AIGetEntityAngleY(AIObjNo),25.0) --10.0) when rotate too slowly, miss target node position!
  if movementfrozen == 0 then
   dy = g_PlayerPosY - (g_Entity[e]['y']+50)
   local stopifclosetofinaldest = 0
   if ai_bot_state[e] ~= ai_state_patrol then
   ...
   if stopifclosetofinaldest == 0 then
    tDistX = x - g_Entity[e]['x']
    tDistY = y - g_Entity[e]['y']
    tDistZ = z - g_Entity[e]['z']
    stopifclosetofinaldest = math.sqrt(math.abs(tDistX*tDistX)+math.abs(tDistY*tDistY)+math.abs(tDistZ*tDistZ))
	if stopifclosetofinaldest < 50 then
     stopifclosetofinaldest = 1
	else
     stopifclosetofinaldest = 0
	end
...
function module_combatcore.soundawareness(e,AIObjNo)
 if AIGetEntityHeardSound(AIObjNo) == 1 then
  ...
  if botState == ai_state_idle or botState == ai_state_hurt then     
   ai_bot_sighting[e] = Timer()+500    
  elseif botState == ai_state_patrol then
   ai_bot_state[e] = ai_state_startidle
   ai_bot_sighting[e] = Timer()+500
  else
   ai_bot_sighting[e] = 0
 ...
 if ai_bot_sighting[e] > 0 and Timer() < ai_bot_sighting[e] then
  RotateToPlayerSlowly( e, 10.0 )
 end
end
