...
bool    ImGui_ImplWin32_Init(void* hwnd)
{
	...
	//PE: Currently we cant run without ImGuiBackendFlags_PlatformHasViewports
	//PE: As it change the mouse cords to be relative to the window position.
	//PE: We cant do this as we now run a simpler system "Lee: ... and a single universal understanding of this helps with coordinates..."
...
{
	if (g_hWnd == 0)
		return true; //PE: Imgui not setup yet.

	ImGuiIO& io = ImGui::GetIO();
	if (io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange)
		return false;
	ImGuiMouseCursor imgui_cursor = ImGui::GetMouseCursor();
	if (imgui_cursor == ImGuiMouseCursor_None || imgui_cursor == ImGuiMouseCursor_Arrow || io.MouseDrawCursor)
	{
		//PE: Allow windows to control mouse pointer.
		return true;
	}
	return false;
}
...
HCURSOR custom_panning = NULL;
HCURSOR custom_hand = NULL;
static bool ImGui_ImplWin32_UpdateMouseCursor()
{
    ...
    else
    {
        ...
        case ImGuiMouseCursor_Hand:
		{
			...
			{
				//Only load one time.
				custom_hand = LoadCursorFromFileA("editors\\uiv3\\hand-open-white.cur");
			}
			if (custom_hand)
				hCursor = custom_hand;
			else
		...
		case ImGuiMouseCursor_Pan:
		{
			if (!custom_panning)
			{
				//Only load one time.
				custom_panning = LoadCursorFromFileA("editors\\uiv3\\hand-closed-white.cur");
			}
			if(custom_panning)
				hCursor = custom_panning;
			else
...
LRESULT ImGui_ImplWin32_WndProcHandler_NewPlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
