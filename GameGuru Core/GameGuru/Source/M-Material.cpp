...
#define MAXMATERIALSTARTCLONESOUNDS 1
...
void material_loadsounds ( int iInitial )
{
	...
	if ( SoundExist(g.explodesoundoffset) == 0 ) 
	{
		...
		for ( t.t = 1 ; t.t <= MAXMATERIALSTARTCLONESOUNDS; t.t++ ) CloneSound (  g.explodesoundoffset+t.t,g.explodesoundoffset );
	}
				...
				for ( t.msoundtypes = 0; t.msoundtypes <= iSoundTypesFullSupport; t.msoundtypes++ )
				{
					...
					if (  FileExist(t.snd_s.Get()) == 1 && t.msoundassign == 0 ) 
					{
						...
						if (  t.tbase<g.materialsoundoffsetend-5 ) 
						{
							...
							//PE: This has been moved , and will only be done if needed.
							//PE: Reduce sound loadtime from 10 sec to 3 sec in Classic.
//							for ( t.tclones = 1 ; t.tclones<= MAXMATERIALSTARTCLONESOUNDS; t.tclones++ )
//							{
...
void material_loadplayersounds ( void )
{
	...
	if ( SoundExist(t.playercontrol.soundstartindex+1) == 0 || g.gplayerstyle_s != t.tplayerstyle_s ) 
	{
		...
		/* do not need these, and the LPSTR = cstr would never have worked)
...
void material_triggersound_core ( int iSoundID )
{
	if (iSoundID > 0 && SoundExist(iSoundID) == 1)
	{
		...
		SetSoundVolume(iSoundID, (60.0 + (t.tvol_f*0.4)) * t.audioVolume.soundFloat);
