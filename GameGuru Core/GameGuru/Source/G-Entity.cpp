...
void entity_reset_defaults(void)
{
	...
	t.entityelement[t.e].floorposy = 0.0f;
			...
			if ( t.entityelement[t.ee].explodefusetime>0 ) 
			{
				if ( Timer()>t.entityelement[t.ee].explodefusetime ) 
				{
					...
					for ( t.tscanexp = 0 ; t.tscanexp<=  4; t.tscanexp++ )
					{
						...
						{
							CloneSound(g.explodesoundoffset + t.tscanexp, g.explodesoundoffset); //Clone original sound.
						}
...
void entity_converttoinstance ( void )
{
	...
	if ( t.entityelement[t.tte].isclone == 1 ) 
	{
		...
		if ( t.tobj>0 ) 
		{
			...
			// 100921 - Early exit dont work, ragdoll not removed, chars facing the wrong way, must restore everything.
			...
			if ( ObjectExist(t.tobj) == 1 ) // && bUniqueSpecularOrUV == false ) 
			{
				...
				#else
				if ( t.entityprofile[t.entityelement[t.tte].bankindex].cpuanims==0 && bUniqueSpecularOrUV == false )
				#endif
...
void entity_updatelightobj ( int e, int obj )
{
	...
	if (e > 0 && obj > 0)
	{
		...
		//if (ilightIndex >= 0 && ilightIndex < t.infinilight.size())
		//{
		//	SetObjectDiffuse(obj, Rgb(t.infinilight[ilightIndex].colrgb.r, t.infinilight[ilightIndex].colrgb.g, t.infinilight[ilightIndex].colrgb.b));
		//}
...
void entity_prepareobj ( void )
{
	...
	if (  ObjectExist(t.tobj) == 1 ) 
	{
		...
		SetObjectArtFlags ( t.tobj, dwArtFlags, 0.0f, t.entityprofile[t.tentid].parallaxstrength);
...
void entity_resettodefaultanimation ( void )
{
	...
	if (  t.tte != -1 ) 
	{
		if (  t.entityelement[t.tte].isclone == 1 )
		{
			...
			{
				if (t.entityprofile[t.tentid].startanimingame > 0 && t.entityprofile[t.tentid].animmax>0 ) {
					t.q = t.entityprofile[t.tentid].startanimingame - 1;
