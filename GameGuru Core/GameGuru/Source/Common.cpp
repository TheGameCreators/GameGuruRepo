...
int common_isserialcodevalid ( LPSTR pSerialCode, LPSTR pOptionalUserName )
{
	...
	// alpha builds expire before September
	...
	// confirm validity of serial code/cloud key
		...
		// if cleverbooks, can only authenticate through cleverbooks server
		if ( pOptionalUserName != NULL )
		{
			...
			//}
			strcpy(m_szPostData, "{");
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, "email");
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, ":");
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, pOptionalUserName);
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, ",");
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, "password");
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, ":");
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, pSerialCode);
			strcat(m_szPostData, pSM);
			strcat(m_szPostData, "}");
			DWORD dwDataReturnedSize = 0;
			LPSTR pDataReturned = NULL;
			char szJSONHeader[1024];
			sprintf(szJSONHeader, "Accept: application/json\nContent-Type: application/json");
			...
			UINT iError = OpenURLForGETPOST("augmented-classroom.com", &pDataReturned, &dwDataReturnedSize, szJSONHeader, m_szPostData, "POST", "/apiv2/product/allowed/vrquest");
			if (iError <= 0 && *pDataReturned != 0 && strchr(pDataReturned, '{') != 0)
			{
				...
				char pErrorMarker[1024];
				strcpy(pErrorMarker, pSM);
				strcat(pErrorMarker, "result");
				strcat(pErrorMarker, pSM);
				strcat(pErrorMarker, ":");
				LPSTR pFindResultToken = strstr (pDataReturned, pErrorMarker);
				if (pFindResultToken)
				{
					char pTrueOrFalse[1024];
					strcpy(pTrueOrFalse, "");
					LPSTR pComma = strstr(pFindResultToken, ",");
					if (pComma)
					{
						int iLengthOfResult = pComma - pFindResultToken;
						memcpy (pTrueOrFalse, pFindResultToken, iLengthOfResult);
						LPSTR pIsTrue = strstr(pTrueOrFalse, "true");
						if (pIsTrue)
						{
				...
				else
				{
					...
					strcpy(pErrorMarker, pSM);
					strcat(pErrorMarker, "error");
					strcat(pErrorMarker, pSM);
					char* pMessageValue = strstr (pDataReturned, pErrorMarker);
					if (pMessageValue)
					{
						// get the error desc part
						pMessageValue += strlen("error")+2+1;
						strcpy (g_pCloudKeyErrorString, pMessageValue);
						LPSTR pLastChar = g_pCloudKeyErrorString+strlen(g_pCloudKeyErrorString)-1;
						if ( *pLastChar == '}') *pLastChar = 0;

						// if NO error, this means the email and password are valid, but the user is NOT licensed for VR QUEST!
						strcpy (g_pCloudKeyErrorString, "You are not licensed to use VR Quest. Please contact Cleverbooks support.");
					}
					else
					{
						strcpy (g_pCloudKeyErrorString, pDataReturned);
					}
					iValidCode = -1;
				}
			}
			else
			{
				...
				strcpy (g_pCloudKeyErrorString, "No Internet Connection - Cannot Authenticate Access");
				iValidCode = -1;
			}
		}
		else
		{
			// VRQ Standalone Cloud Key Auth
			// try both V3 Home Edition uses AppID 2 (V2/V3 Site Licenses uses AppID 1)
			for (int iSiteOrHome = 1; iSiteOrHome <= 2; iSiteOrHome++)
			{
				...
				if (iSiteOrHome == 1) strcat(szGetData, "1"); // V2/V3 Site License
				if (iSiteOrHome == 2) strcat(szGetData, "2"); // V3 Home Edition
				...
				if (strstr (pDataReturned, "Send Request failed") == NULL)
				{
					...
					if (strstr (pStatusValue, "success") != NULL)
					{
						...
						if (bIsKeyValid == true)
						{
							...
							// record if school or home edition
							if (iSiteOrHome == 2) g_bCloudKeyIsHomeEdition = true;

							// optionally, get the expiry date from the return message
							char* pExpiresStart = strstr (pChop, "expires");
							if (pExpiresStart)
							{
								pExpiresStart += 10; //expires":"
								memcpy(g_pCloudKeyExpiresDate, pExpiresStart, 4 + 1 + 2 + 1 + 2);
								g_pCloudKeyExpiresDate[4 + 1 + 2 + 1 + 2] = 0;
							}

							// can leave as key works for one of the versions
							break;
						}
						else
						{
	...
	#endif // CLOUDKEY endif
	#endif // ALPHAEXPIRE endif
...
void common_writeserialcode ( LPSTR pCode, LPSTR pOptionalUsername )
{
	...
	{
		strcat (pAbsFilePath, "\\cleverbooksmode.ini");
		if (FileExist(pAbsFilePath) == 1) DeleteFileA (pAbsFilePath);
		if (FileOpen(1) == 1) CloseFile (1);
		OpenToWrite (1, pAbsFilePath);
		WriteString (1, pOptionalUsername);
		WriteString (1, pCode);
		CloseFile (1);
	}
	else
	{
		...
		{
			#ifdef VRTECH
			strcat(pAbsFilePath, "\\ggcontrolmode.ini");
			#else
