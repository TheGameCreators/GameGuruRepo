...
void ravey_particles_set_life( int iID, int iSpawnRate, float iLifeMin, float iLifeMax, int iMaxParticles, int iOnDeathAction )
{
	if (t.ravey_particle_emitters[iID].inUse == 1)
	{
		...
		t.ravey_particle_emitters[iID].onDeathAction = iOnDeathAction;  //  0 - default, 1 - 'split'
	}
}
		...
		if ( this_emitter->inUse ) 
		{
				  ...
				  this_emitter->numParticles < this_emitter->maxParticles )
			{
				...
				for ( t.tNewParticleCount = 0 ; t.tNewParticleCount <  t.tAmountToMake; t.tNewParticleCount++ )
				{
					...
					++this_emitter->zclashfixcounter;
					if ( this_emitter->zclashfixcounter > 10 )  this_emitter->zclashfixcounter = 0;
					MoveObject( t.tobj, this_emitter->zclashfixcounter * 0.1f );
					//  animation
					...
					{
						ZRotateObject ( t.tobj, (float) Rnd(360) );
					}
		...
		if ( this_particle->inUse ) 
		{
			...
			else
			{
				...
				if ( this_particle->isAnimated == 1 ) 
				{
					...
					if ( this_particle->frame < (float) this_particle->startFrame )  this_particle->frame  =  (float) this_particle->startFrame;
				...
				this_particle->x += fwindVectX * g.ravey_particles_time_passed;
				...
				this_particle->z += fwindVectZ * g.ravey_particles_time_passed;

				PositionObject( t.tobj, this_particle->x, this_particle->y, this_particle->z );

				// currently only rotates in Z axis *after* roating to face camera
				// TBD  - interpret x,y,z rotations as quaternion and use to rotate object?
				// TBD2 - Instead of rotate to camera, have fixed to plane? (i.e. so we can display 'particles' on the surface of an entity)
				...
				while ( t.rotz > 360 ) { t.rotz = t.rotz - 360; }
				while ( t.rotz < 0 ) { t.rotz = t.rotz + 360; }
