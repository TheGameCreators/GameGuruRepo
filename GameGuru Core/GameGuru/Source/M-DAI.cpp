...
void darkai_preparedata ( void )
{
	...
	//PE: Check if we got a cached obs file.
	if (t.aisystem.generateobs != 0 && t.game.gameisexe == 1)
	{
		bool bLoadOBSFileFromCache(void);
		if (bLoadOBSFileFromCache())
		{
			t.aisystem.generateobs = 0;
		}
	}
...
void darkai_completeobstacles ( void )
{
	...
	if (  t.aisystem.generateobs == 1 ) 
	{
		...
		//PE: If standalone save a copy of obs to cache folder.
		#ifdef PRODUCTCLASSIC
		...
		if (  t.te>0 ) 
		{
			...
			if (  t.tgunid>0 && t.tcannotfirenow == 0 ) 
			{
				...
				if (  t.charanimstate.firerateaccumilator<0.0 ) 
				{
					...
					//PE: Update. or it has no effect. https://github.com/TheGameCreators/GameGuruRepo/issues/1395
					if (t.tcharanimindex > 0)
						t.charanimstates[t.tcharanimindex].firerateaccumilator = t.charanimstate.firerateaccumilator;
...
void darkai_mouthandheadtracking ( void )
{
	...
	SlerpFrameAnimToUserFrame ( pCharObject, iFinalFrameToUse, t.charanimstate.ccpo.speak.fSmouthDataSpeedToNextShape );

	// Neck Bone adjust as part of manual and regular anim calculation
	if (t.charanimstate.ccpo.settings.iNeckBone > 0)
	{
		RotateLimb ( iCharObj, t.charanimstate.ccpo.settings.iNeckBone, t.charanimstate.neckRightAndLeft, t.charanimstate.neckUpAndDown, 0 );
	}
}
#endif
...
void char_loop ( void )
{
	...
	// Update anim system for smoothing
