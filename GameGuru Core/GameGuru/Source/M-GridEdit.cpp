...
#ifndef PRODUCTCLASSICIMGUI
extern ISpObjectToken * CCP_SelectedToken; //cyb
extern LPSTR pCCPVoiceSet; //cyb
extern char CCP_SpeakText[1024]; //cyb
extern wchar_t CCP_SpeakText_w[1024]; //cyb
extern int CCP_Speak_Rate; //cyb
#else
ISpObjectToken * CCP_SelectedToken; //cyb
LPSTR pCCPVoiceSet; //cyb
char CCP_SpeakText[1024]; //cyb
wchar_t CCP_SpeakText_w[1024]; //cyb
int CCP_Speak_Rate; //cyb
#endif
...
char * imgui_setpropertyfile2(int group, char* data_s, char* field_s, char* desc_s, char* within_s); //moved //cyb
...
void mapeditorexecutable_loop(void)
{
	...
	if (bImGuiFrameState) {
		...
		#ifndef PRODUCTCLASSICIMGUI
		if(g_bCharacterCreatorPlusActivated) //cyb
		#endif
...
//					#endif
				...
				if (ImGui::BeginMenu("Change Color Scheme")) 
				{
					...
					if (ImGui::MenuItem("Light Style")) { // Classic Light Style
						myLightStyle(NULL);
						pref.current_style = 3;
					}
					#endif
...
#else
					ExecuteFile("https://www.game-guru.com/classic", "", "", 0);
#endif
				}
				#ifndef DISABLETUTORIALS
...
//						sprintf(tut.cStepText[tut.iSteps-1], "Score: %.0f", tut.fScore);
		...
		if (bExport_SaveToGameCloud_Window)
		{
			...
			if (ImGui::BeginPopupModal("Save To Level Cloud##SaveToLevelCloudWindow", &bExport_SaveToGameCloud_Window, 0)) 
			{
				...
				#else
				std::vector<cstr> g_gamecloud_gamelist; //cyb
				#endif
