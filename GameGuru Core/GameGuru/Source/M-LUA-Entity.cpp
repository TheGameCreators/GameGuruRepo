...
void entity_lua_moveforward_core_ex ( float fActualMoveUnitsOverride, float fForceWorldX, float fForceWorldZ )
{
	// resets
	float fLastX = 0;
	float fLastZ = 0;
	...
	else
	{
		...
		fNewX = NewXValue(t.entityelement[t.e].x,t.entityelement[t.e].ry,t.ttv_f);
		fNewZ = NewZValue(t.entityelement[t.e].z,t.entityelement[t.e].ry,t.ttv_f);
	}
...
{
	entity_lua_moveforward_core_ex (fActualMoveUnitsOverride, 0.0f, 0.0f);
}
...
void entity_lua_setanimationname ( void )
{
	...
	LPSTR pFindThisName = t.s_s.Get();
	cStr lowercase_s = t.s_s.Lower();
	sObject* pObject = GetObjectData(t.entityelement[t.e].obj);
	if (pObject)
	{
		bool bFoundBest = false;
		for (int iSearchPatterns = 0; iSearchPatterns < 4 && bFoundBest==false; iSearchPatterns++)
		{
	...
	// strange issue - need to shift frames to avoid a 'glitch' (when zombie walk moving object position) ?!?
	//fFoundStart -= 1; but this messes up other anims which DO start on actual start frame!!
	fFoundFinish -= 1;
...
void entity_lua_playorloopanimation ( void )
{
	...
	if ( t.obj>0 ) 
	{
		if ( ObjectExist(t.obj) == 1 ) 
		{
			...
			#ifdef VRTECH
			//smoothanimtriggerrev(t.obj,t.ttstart,t.ttfinish,10.0,0,t.playflag); some models cannot transition between frames!! No interpolation now.
...
void entity_lua_aimatplayer (void)
{
	entity_lua_findcharanimstate();
	if (t.tcharanimindex != -1)
	{
