...
void weapon_projectile_init ( void )
{
	...
	//PE: We use generic now, so moved this.	
	// free thread loader for next batch of work
...
void weapon_projectile_make ( bool bUsingVRForAngle )
{
	...
	if (  t.tobj > 0 ) 
	{
		if (  t.WeaponProjectileBase[t.tProjectileType].usingRealPhysics>0 ) 
		{
			...
			float fObjStartX = ObjectPositionX(t.tobj);
			float fObjStartY = ObjectPositionY(t.tobj);
			float fObjStartZ = ObjectPositionZ(t.tobj);
			float fForceProjectileTurnLeft = t.WeaponProjectileBase[t.tProjectileType].usingRealPhysicsAdjustYAngle;
			...
			t.WeaponProjectile[t.tNewProj].xSpeed_f=ObjectPositionX(t.tobj) - fObjStartX;
			t.WeaponProjectile[t.tNewProj].ySpeed_f=ObjectPositionY(t.tobj) - fObjStartY;
			t.WeaponProjectile[t.tNewProj].zSpeed_f=ObjectPositionZ(t.tobj) - fObjStartZ;
			MoveObject (  t.tobj,-10.0 );
			...
			t.tidiv_f = 1.0f;// 0.2f; speed fix above must restore speed of the projectile
