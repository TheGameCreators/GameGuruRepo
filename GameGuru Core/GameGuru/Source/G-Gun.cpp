...
void gun_activategunsfromentities ( void )
{
	...
	for ( t.e = 1 ; t.e<=  g.entityelementlist; t.e++ )
	{
		...
		if (  t.entid>0 ) 
		{
			//  LEE ALSO CHECK FOR START MARKER
...
void gun_manager ( void )
{
	...
	if ( t.playercontrol.thirdperson.enabled == 1 && t.gunid == 0 )
	{
...
void gun_change ( void )
{
	...
	if ( g.autoloadgun != -1 && g.autoloadgun <= ArrayCount(t.gun) ) 
	{
		...
		t.currentguncolr=-1 ; lighting_applyplayerlighting ( );
...
bool gun_getstartandfinish ( void )
{
...
void gun_control ( void )
{
	...
	if (  g.firemodes[t.gunid][g.firemode].settings.hasempty == 1 && g.firemodes[t.gunid][g.firemode].settings.isempty == 1 ) 
	{
		...
		if ( bReallyRunning==false && t.playercontrol.usingrun != -1 ) 
		{
			...
			if (  t.gunmode >= 21 && t.gunmode <= 26  )  
			{
				t.gunmode = 21;
				//t.gunmodetransitionoverride = 0;
			}
		}
		if ( bReallyRunning == true && t.playercontrol.usingrun != 1 ) 
		{
			...
			{
				t.gunmode = 21;
				//t.gunmodetransitionoverride = 10; // 310518 - does not work if limbs A-B trajectory is too gimbled
			}
		}
	...
	else
	{
		...
		if ( bReallyRunning == true && t.playercontrol.usingrun != 1 ) 
		{
			...
			{
				t.gunmode = 21;
				//t.gunmodetransitionoverride = 10;
			}
		}
	...
	if (  t.gunmode == 21 ) 
	{
		t.gunmode=22;
		StopObject (  t.currentgunobj );
		if ( t.gunmodetransitionoverride > 0 )
		{
			SetObjectInterpolation ( t.currentgunobj, t.gunmodetransitionoverride );
			t.guninterp = 100 / t.gunmodetransitionoverride;
			t.gunmodetransitionoverride = 0;
		}
		else
		{
	...
	if (  t.gunmode == 101 ) 
	{
		...
		else
		{
			if (  t.gunandmelee.pressedtrigger == 0 ) 
			{
				...
				// dryfire animation (only if was not moving)
				if ( t.gunmodelast < 21 || t.gunmodelast > 26 )
				{
					if (  t.gunzoommode == 0 ) 
					{
	...
	if ( t.gunmode == 104 ) 
	{
		...
		if (  t.weaponammo[g.weaponammoindex+g.ammooffset]>0 ) 
		{
			...
			if (  g.gunbrasscount <= 0 && t.gun[t.gunid].settings.brasslimb != -1 ) { bBrassEjected = true; t.gunbrass = 1  ; g.gunbrasscount = g.firemodes[t.gunid][g.firemode].settings.firerate/2; }
...
void gunmode121_cancel ( void )
{
	...
	if (  t.gunmode == 132 ) 
	{
		t.currentgunanimspeed_f=g.timeelapsed_f*t.genericgunanimspeed_f;
		SetObjectSpeed (  t.currentgunobj,t.currentgunanimspeed_f );
		if (  GetFrame(t.currentgunobj) >= t.gshow.e  )  t.gunmode = 5;
		if (  GetFrame(t.currentgunobj)<t.gshow.s  )  t.gunmode = 5;
	}
}
...
void gun_flash ( void )
{
	...
	if (  g.firemodes[t.gunid][g.firemode].settings.simplezoom  !=  0 && t.gunzoommode  !=  0 && g.firemodes[t.gunid][g.firemode].settings.simplezoomflash  ==  1 ) 
	{
		PositionObject (  g.hudbankoffset+5,g.firemodes[t.gunid][g.firemode].settings.zoommuzzlex_f+g.gunlagX_f+g.gunOffsetX_f,g.firemodes[t.gunid][g.firemode].settings.zoommuzzley_f+g.gunlagY_f+g.gunOffsetY_f,g.firemodes[t.gunid][g.firemode].settings.zoommuzzlez_f );
	}
	else
	{
		PositionObject (  g.hudbankoffset+5,g.firemodes[t.gunid][g.firemode].settings.muzzlex_f+g.gunlagX_f+g.gunOffsetX_f,g.firemodes[t.gunid][g.firemode].settings.muzzley_f+g.gunlagY_f+g.gunOffsetY_f,g.firemodes[t.gunid][g.firemode].settings.muzzlez_f );
	}
	if (  t.gunflash == 1 ) 
	{
		//  fire flash init
...
void gun_shoot_oneray ( void )
{
	...
	if ( t.playercontrol.thirdperson.enabled == 1 )
	{
		...
		MoveObject ( g.hudbankoffset+3, 7000.0f );
	}
	else
	{
...
void gun_load ( void )
{
	...
	// LoadEffect ( (default is weapon_basic shader) )
