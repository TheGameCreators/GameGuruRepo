...
void importer_init ( void )
{
	...
	if (g_bCameraInSkyForImporter == false)
	{
		...
		SetCameraRange(0, 1.0f, 300000.0f);
...
void importer_free ( void )
{
	...
	if ( g_bCameraInSkyForImporter == true )
	{
		...
		if ( t.editorfreeflight.mode == 1 )
			SetCameraRange ( 0, t.tcamneardistance_f, 300000 );
		else
	...
	if (bImporter_Window && t.importer.importerActive == 1)
	{
		...
		if (iImporterGenerateThumb <= 5)
		{
			if (iImporterGenerateThumb == 5)
			{
				...
				if (GetImageExistEx(g.importermenuimageoffset + 50)) DeleteImage(g.importermenuimageoffset + 50);
		...
		if (ImGui::StyleCollapsingHeader("Name", ImGuiTreeNodeFlags_DefaultOpen)) 
		{
			...
			//	if(ImGui::ImgBtn(g.importermenuimageoffset + 50, ImVec2(media_icon_size, media_icon_size), ImColor(0, 0, 0, 255), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 255), ImColor(180, 180, 180, 255),2,0,0,0,true,false)) {
			//		iImporterGenerateThumb = 0;
			//	}
			//}
...
#else
	if (t.slidersmenuvalue[t.importer.properties1Index][7].value == 3) { //is uber
																		 //PE: If model already have firespot (from artist) this will overwrite it, so only if uber. (assume uber dont have firespot).
...
void importer_save_entity ( char *filename )
{
	...
	else
	{
		if (GetImageExistEx(g.importermenuimageoffset + 50)) 
		{
			...
			SaveImage(pRealSavethumb, g.importermenuimageoffset + 50);
		}
		else 
		{
			if (FileExist(pRealSavethumb) == 1) DeleteAFile(pRealSavethumb);
...
void import_generate_thumb(void)
{
	...
	if (m_mycam)
	{
		...
		GrabImage(g.importermenuimageoffset + 50, gi_rectw, gi_recth, gi_rectw + thumbnail_dimenions, gi_recth + thumbnail_dimenions, 0);
