...
#ifndef FASTBULLETPHYSICS
btScalar AvgtimeStep = 0.0f;
btScalar AvgtimeStep2 = 0.0f;
bool bPhysicsRunningAt120FPS = true;
#endif

extern int iTotalPenetrationRecoveryAttempts;
...
int	btDiscreteDynamicsWorld::stepSimulation( btScalar timeStep,int maxSubSteps, btScalar fixedTimeStep)
{
	...
	if (maxSubSteps)
	{
		...
		//PE: We run at 60fps and need 2 opdates to be even as we use a fixedtime at 120fps.
		//PE: Adjust depending on, if we can affort it. When running at full speed allow full physics.
		//PE: This is a workaround the real problem is the btKinematicCharacterController, it is bad and really need to be replaced with something better.

		#ifndef FASTBULLETPHYSICS
		btScalar timeavg = (timeStep + AvgtimeStep + AvgtimeStep2) * 0.3333;

		//PE: Some people is actually running everything at very low fps, so only activate it we are actually stuck.

		// > 15 = 3 calls to internalSingleStepSimulation all failing , so we are stuck. move world in 1/60.
		//if (iTotalPenetrationRecoveryAttempts > 15 || timeavg > (1.0f / 25.0f))
		if (iTotalPenetrationRecoveryAttempts >= 15 || timeavg > (1.0f / 25.0f))
		{
			//PE: For now until we have a new char controller, just set it to max 1 simulation so we dont drain everything.
			numSimulationSubSteps = 1;

			//Cant affort it really behind.
			//numSimulationSubSteps = 1;
			//Set fixedTimeStep to 1.0/60. This moves the world the same as two calls at 1/120
			//fixedTimeStep = (1.0f / 60.0f);
			//bPhysicsRunningAt120FPS = false;
		}
		//else if (timeavg > (1.0f / 55.0f))
		//{
		//	//Cant affort it.
		//	if(numSimulationSubSteps > 2)
		//		numSimulationSubSteps = 2;
		//}
		AvgtimeStep = AvgtimeStep2;
		AvgtimeStep2 = timeStep;
		#endif	
	} else
	{
		//variable timestep
	...
	if (numSimulationSubSteps)
	{
		...
		iTotalPenetrationRecoveryAttempts = 0;
