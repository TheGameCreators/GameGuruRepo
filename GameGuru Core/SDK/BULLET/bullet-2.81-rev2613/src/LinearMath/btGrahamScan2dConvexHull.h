...
inline void GrahamScanConvexHull2D(btAlignedObjectArray<GrahamVector3>& originalPoints, btAlignedObjectArray<GrahamVector3>& hull, const btVector3& normalAxis)
{
	...
	for (int ii=0;ii<originalPoints.size();ii++)
	{
		btScalar projL = originalPoints[ii].dot(axis0);
		btScalar projR = originalPoints[0].dot(axis0);
		if (projL < projR)
		{
			originalPoints.swap(0,ii);
		}
	}

	//also precompute angles
	originalPoints[0].m_angle = -1e30f;
	for (int ii=1;ii<originalPoints.size();ii++)
	{
		btVector3 xvec = axis0;
		btVector3 ar = originalPoints[ii]-originalPoints[0];
		originalPoints[ii].m_angle = btCross(xvec, ar).dot(normalAxis) / ar.length();
	}
