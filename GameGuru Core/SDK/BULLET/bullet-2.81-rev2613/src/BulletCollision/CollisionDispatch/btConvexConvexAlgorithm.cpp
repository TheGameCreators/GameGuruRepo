...
void btConvexConvexAlgorithm ::processCollision (const btCollisionObjectWrapper* body0Wrap,const btCollisionObjectWrapper* body1Wrap,const btDispatcherInfo& dispatchInfo,btManifoldResult* resultOut)
{
	...
	// if one of the colliders is a capsule (character), store the 'material value' in its body so I can get it later
	if ( min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE || min1->getShapeType() == CAPSULE_SHAPE_PROXYTYPE )
	{
		int iMaterialIndex = 0;
		if ( min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE )
		{
			if ( min1->getShapeType() != TRIANGLE_SHAPE_PROXYTYPE )
			{
				iMaterialIndex = (int)min1->getUserPointer();
			}
			((btCollisionObject*)body0Wrap->getCollisionObject())->setUserPointer((void*)iMaterialIndex);
		}
		else
		{
			if ( min0->getShapeType() != TRIANGLE_SHAPE_PROXYTYPE )
			{
				iMaterialIndex = (int)min0->getUserPointer();
			}
			((btCollisionObject*)body1Wrap->getCollisionObject())->setUserPointer((void*)iMaterialIndex);
		}
	}
