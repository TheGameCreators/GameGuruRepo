...
volatile float sampleFreqMadgwick = sampleFreqDef;
...
void MadgwickSetBeta( float b ){
	beta = b;
}

void MadgwickSetSampleFreq( float freq ){
	sampleFreqMadgwick = freq;
}
...
void MadgwickSetFromAxes(float ax, float ay, float az, float mx, float my, float mz) {
	float recipNorm;
	float halfAngle;
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	q0 += qDot1 * (1.0f / sampleFreqMadgwick);
	q1 += qDot2 * (1.0f / sampleFreqMadgwick);
	q2 += qDot3 * (1.0f / sampleFreqMadgwick);
	q3 += qDot4 * (1.0f / sampleFreqMadgwick);
